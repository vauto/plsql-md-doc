...
const { SyntaxNode, IdentifierSyntaxNode, Annotation, LiteralSyntaxNode } = require("./syntax")
 ...
 *
 * @typedef {import('./name').NamedSyntaxNode} NamedSyntaxNode
 */
...
class CodeContext {
  ...
  resolveItem(node) {
    ...
    // Do NOT strip quotes here, only in resolveContent.
  ...
  *#doResolveContent(...content) {
    for (const item of content) {
      ...
      if (item instanceof SyntaxNode) {
        ...
        yield* this.#doResolveContent(...item)
      }
    }
  }
  ...
  resolveContent(...content) {
    switch (content.length) {
      case 1:
        if (content[0] instanceof LiteralSyntaxNode) {
