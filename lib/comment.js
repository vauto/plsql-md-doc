 ...
 * @typedef {string|URL|Tag} Content
 */
...
class CodeContext {
  ...
  *#resolveInlineTagContent(item) {
    ...
    if (item.hasLeadingTrivia) {
      yield item.leadingTrivia.join('')
    }
    yield Tag.from(item.tag, this)
    if (item.hasTrailingTrivia) {
      yield item.trailingTrivia.join('')
    }
  }
...
class IncludeTag extends Tag {
  ...
  constructor(syntax, context) {
    ...
    // Resolve path
    this.url = context.resolveUrl(syntax.url)

    // Resolve content (LATER: async)
    if (this.url) {
      switch (this.url.protocol) {
        case 'file:':
          this.content = [fs.readFileSync(fileURLToPath(this.url), 'utf8')]
          break
        default:
          console.warn(`IncludeTag: '${this.url.protocol}' URLs are currently unsupported`)
          break
      }
    }
  }
