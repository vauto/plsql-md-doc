...
const { mustBeInstanceOf, mustBeArray, InvalidNumberOfArgumentsError, ArgumentValueError, mustBeObject } = require('./guards')
const {
...
const { PlsqlMemberName, PlsqlUniqueId, PlsqlIdentifier } = require('./plsql/name')
 ...
 *
 * @typedef { string | URL | Tag | ItemName | SyntaxNode } ContentItem
...
class Annotation {
   ...
   * @param {CodeContext} parent
   */
  constructor(/** @type {AnnotationNode|Tag} */ nodeOrTag, /** @type {CodeContext} */ parent) {
    switch (true) {
      case nodeOrTag instanceof AnnotationNode:
        {
          const node = nodeOrTag
          this.parent = parent
          this.name = node.kind // yes, we want the node's kind for the name
          this.messageNode = node.message
        ...
        break
      case nodeOrTag instanceof Tag:
        {
          const [tag, context] = arguments
          this.parent = context
          this.name = tag.kind // yes, we want the tag's kind for the name
          this.messageNode = tag
        }
        break
      default:
        throw new ArgumentValueError('node|tag', nodeOrTag, 'Value must be an instance of one of the following: AnnotationNode, Tag')
    }
 ...
 */
class CodeContext {
  /** @type {string} The general kind (e.g., `method`, `type`) */ kind
  /** @type {string} The detailed kind (e.g. `constructor function`).  If not specified, defaults to {@link kind}. */ subkind
  /** @type {CodeContext?} */ parent
  ...
  /**
   * @param {object} params
   * @param {string?} params.kind The general kind (e.g., `method`, `type`)
   * @param {string?} [params.subkind] The detailed kind (e.g. `constructor function`).
  ...
  resolveUrl(node) {
    ...
    if (node.value.match(/^[#?]/)) {
      // URL is just query or fragment, and so is relative to the **output** document,
      // not to the initial.  Keep as-is.
      return node.value
    } else {
  ...
  *#resolveInlineTagContent(item) {
    ...
    // If the outer inline tag contains leading/trailing structured trivia (e.g., whitespace),
    // extract those and return them as well to ensure proper padding.
...
class Description {
  ...
  /**
   * Splits the first sentence from the full text.
   *
   * @remarks
   ...
   * This sentence ends at the first period that is followed by a blank, tab, or line terminator.
   *
   * @remarks
   * Javadoc includes "Block tags" as indicating the terminator, but should not be present at this point in the description,
   * so we only care about period + whitespace.
   *
   * @see https://docs.oracle.com/javase/8/docs/technotes/tools/windows/javadoc.html#JSWOR649
   *
   * @param {string} text
   * @returns {[string, string]}
   */
  static #splitText(text) {
    // Only slice if multiple lines are found.
    const eolIndex = text.indexOf(EOL)
    if (eolIndex < 0) {
      return [text, '']
    }

    const match = reEndOfSentence.exec(text)
    if (match) {
      const [delim, period, whitespace] = match
      const delimIndex = text.indexOf(delim)
      if (delimIndex >= eolIndex - 1 || whitespace.indexOf(EOL) > 0) {
        // Period at or after the EOL (note: if whitespace contains EOL but has more whitespace, it's treated as "at EOL".)
        // That makes it easy.
        return [
          text.substring(0, delimIndex + period.length).trimEnd(), // include the period
          text.substring(delimIndex + delim.length).trimStart()
        ]
      }

      console.assert(whitespace.indexOf(EOL) === -1, 'should have been caught already')

      // End-of-sentence is *before* EOL.
      if (whitespace.length > 1) {
        // Two or more whitespace characters found, but the EOL was *not* part of this whitespace.
        // This is intentional whitespace separation and not, say, someone forgetting to trim whitespace.
        return [
          text.substring(0, delimIndex + period.length).trimEnd(), // include the period
          text.substring(delimIndex + delim.length).trimStart()
        ]
      ...
      // Everything else is ambiguous; fall through.
      console.warnOnce(text, 'check text with period+space', JSON.stringify(text))
    } else if (text.endsWith('.')) {
      // The whole sentence is a period.
      // Do not split.
      return [text, '']
    } else {
      console.warnOnce(text, 'check text without period+space or ending period', JSON.stringify(text))
    }

    // Slice at the EOL.
    return [
      text.substring(0, eolIndex).trimEnd(),
      text.substring(eolIndex + EOL.length).trimStart()
    ]
  }
   ...
   * @overload
   * @param {object} params
   * @param {string} params.summary The summary
   * @param {string} params.body The body
   */
  constructor(/** @type {string|object} */ params) {
    if (typeof params === 'string') {
      this.full = params.trim();

      // Summary should be the first sentence; body is everything else.
      [this.summary, this.body] = Description.#splitText(this.full)

    } else {
      mustBeObject(params)
      this.summary = params.summary
      this.body = params.body
      this.full = Description.#joinStrings(this.summary, this.body)
    }
  }
  ...
  static concat(value1, value2) {
    if (!value2) {
      return Description.from(value1)
    } else if (!value1) {
      return Description.from(value2)
    }

    return Description.from(value1).concat(value2)
  }
  ...
  concat(value) {
    ...
    if (!this.body) {
      // The value is now our body
      return new Description({ summary: this.summary, body: value.full })
    }

    // Concatenate the whole thing.
    return new Description({ summary: this.summary, body: Description.#joinStrings(this.body, value.full) })
  }
...
class Tag {
  ...
  static from(syntax, context) {
    ...
    switch (true) {
      ...
      case syntax instanceof CodeTagSyntaxNode:
        return new CodeTag(syntax, context)
...
class ParamTag extends Tag {
  ...
  constructor(syntax, context) {
    ...
    // Because the Javadoc doesn't resolve PL/SQL identifiers explicitly, we must.
    this.name = PlsqlIdentifier.parse(syntax.name.value)
  }
