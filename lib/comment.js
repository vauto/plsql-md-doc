...
class CodeContext {
  ...
  resolveLink(node) {
    ...
    return node.toStructuredString('T').trim()
  }
  ...
  resolveItem(node) {
    switch (true) {
      ...
      case node instanceof UrlLiteralSyntaxNode:
        return new URL(node.value)
      case node instanceof InlineTagSyntaxNode:
        return node.toStructuredString('T') // FIXME this.resolveItem(node.tag)?
      case node instanceof BraceNamePathExpressionSyntaxNode:
        // FIXME resolve url
        return node.toStructuredString('T')
      case node instanceof BraceContentExpressionSyntaxNode: // unintentional text in braces, render as-is
        return node.toStructuredString('T')
      case node instanceof BraceExpressionSyntaxNode:
        return node.toStructuredString('T')
      case node instanceof BracketExpressionSyntaxNode:
        return node.toStructuredString('T')
      case node instanceof ContentSyntaxNode:
        return node.toStructuredString('T')
      default:
        console.warnOnce(node.kind, `resolve non-content item`, node)
        return node.toStructuredString('T')
    }
  }
  ...
  *#doResolveContent(...content) {
  ...
  resolveContent(...content) {
    switch (content.length) {
      ...
      case 1:
        switch (true) {
          case content[0] instanceof LiteralSyntaxNode:
    ...
    return [...this.#doResolveContent(...content)].join('').trim()
  }
}
...
class ContentTag extends Tag {
  ...
  constructor(syntax, context) {
    ...
    console.assert(context || !syntax.content, `${syntax.textSpan} should have context if there is content`)
    this.content = context?.resolveContent(...syntax.content)
    this.name = 'name' in syntax ? syntax.name : undefined
    this.text = syntax.text
