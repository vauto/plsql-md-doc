...
const { SyntaxNode, Annotation } = require('./syntax')
...
const { PlsqlUnitName, PlsqlMemberName } = require('./plsql/name');
const {
  PlsqlNodeReader,
  ...
  *parseDocCommentSyntax(syntax) {
  ...
  mapParameterToCodeContext = (node, parent) => {
    ...
    const result = {
      ...
      name: Identifier.from(node.name),
  ...
  mapAnnotationToCodeContext = (annotation) => {
    return {
      name: PlsqlMemberName.from(annotation.name),
      parameters: annotation.parameters.map(p => p.value),
      target: PlsqlMemberName.from(annotation.target.name),
