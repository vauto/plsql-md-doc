...
const {
  ...
  ProcedureDeclarationStatementSyntaxNode,
  FunctionDeclarationStatementSyntaxNode,
  ConstructorDeclarationStatementSyntaxNode,
  ObjectMethodDeclarationStatementSyntaxNode
...
class DocumentGenerator {
  ...
  *parseDocCommentSyntax(syntax) {
  ...
  codeFormatters = {
    ...
    [ProcedureDeclarationStatementSyntaxNode]: (/** @type {ProcedureDeclarationStatementSyntaxNode} */ node, format = 'T') => {
    ...
    [FunctionDeclarationStatementSyntaxNode]: (/** @type {FunctionDeclarationStatementSyntaxNode} */ node, format = 'T') => {
      if (node.openParenToken) {
        const tokens = node.tokens
        const throughOpenParen = tokens.slice(0, tokens.indexOf(node.openParenToken))
        const closeToEnd = tokens.slice(tokens.indexOf(node.closeParenToken))
    ...
    [ConstructorDeclarationStatementSyntaxNode]: (/** @type {ConstructorDeclarationStatementSyntaxNode} */ node, format = 'T') => {
      return this.codeFormatters[FunctionDeclarationStatementSyntaxNode](node, format)
    },
    [ObjectMethodDeclarationStatementSyntaxNode]: (/** @type {ObjectMethodDeclarationStatementSyntaxNode} */ node, format = 'T') => {
      return this.codeFormatters[FunctionDeclarationStatementSyntaxNode](node, format)
    }
  }
    ...
    (node, parentContext) => {
      if (node instanceof SubtypeDeclarationStatementSyntaxNode) {
        return {
          ...
          kind: 'subtype',
          name: this.#mapIdentifier(node.name),
          specification: node.baseType?.toString('T'),
          annotations: node.annotations.map(this.mapAnnotationToCodeContext)
        }
      }
    },
    ...
    (node, parentContext) => {
      if (node instanceof RecordTypeDeclarationStatementSyntaxNode) {
        ...
        const result = new CodeContext({
          ...
          kind: 'record',
          name: this.#mapIdentifier(node.name),
          specification: this.codeFormatters[node.constructor](node),
