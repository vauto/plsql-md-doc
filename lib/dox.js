...
const { PlsqlNodeReader, TypeDeclarationStatementSyntaxNode, ParameterDeclarationExpressionSyntaxNode } = require('./plsql/syntax');
...
class DocumentGenerator {
  ...
  *parseCommentSyntax(text) {
  ...
  parseComment(token, code, parentContext) {
    ...
    return new Comment({ code, token, nodes, context: this.parseCodeContext(code, parentContext) })
  }
  ...
  /**
   *
   * @param {ParameterDeclarationExpressionSyntaxNode} paramNode
   * @returns
   */
  mapParameterToCodeContext = (paramNode) => {
    return {
      id: paramNode.name.toString('V'),
      name: paramNode.name.toString('T'),
      mode: paramNode.mode.join(' ') || 'IN',
      type: paramNode.type.toString('T'),
      defaultValue: paramNode.defaultValue?.toString('T'),
      optional: !!paramNode.defaultValue
    }
  }
