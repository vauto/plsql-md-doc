...
class DocumentGenerator {
  ...
  *parseDocCommentSyntax(syntax) {
  ...
  codeFormatters = {
    [RecordTypeDeclarationStatementSyntaxNode]: (node, format = 'T') => {
        ...
        const fieldNamePadding = this.indentSize * Math.ceil((1 + node.fields.reduce((max, field) => Math.max(max, field.name.length), 0)) / this.indentSize)

        return [
          `${stringifyTokenArray(throughOpenParen, format)} ${node.openParenToken}`,
          node.fields
            .map(f => f.toString(format).replace(/ /, ' '.padStart(fieldNamePadding - f.name.length)))
