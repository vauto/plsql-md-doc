...
exports.parseTokens = function* (js, options) {
  ...
  let startIndex = 0, pos = new Position(), startPos = pos.clone()

  SCAN:
  while (i < len) {
    console.assert(startPos.position === startIndex && pos.position === i, `oops start (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)
    if (js[i] === '/' && js[i + 1] === '*') {
      // Multiline comment detected.
      // Emit previous token (as code)
      if (startIndex < i) {
        yield { text: js.substring(startIndex, i), type: 'code', start: startPos, end: pos.clone() }
        startIndex = i
        startPos = pos.clone()
        console.assert(startPos.position === startIndex && pos.position === i, `oops code -> mcomment (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)
      }
      ...
      if (i >= 0) {
        i += 2
        yield { text: js.substring(startIndex, i), type: 'mcomment', start: startPos, end: pos.clone() }
        startIndex = i
        startPos = pos.clone()
        console.assert(startPos.position === startIndex && pos.position === i, `oops mcomment (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)
        ...
        startIndex = i = len
        startPos = pos.clone()
        console.assert(startPos.position === startIndex && pos.position === i, `oops mcomment-eof (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)
        break SCAN
      }
      ...
      if (i >= 0) {
        i++
        yield { text: js.substring(startIndex, i), type: 'scomment', start: startPos, end: pos.clone() }
        startIndex = i
        startPos = pos.clone()
        console.assert(startPos.position === startIndex && pos.position === i, `oops scomment (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)

        continue SCAN
      } else {
        // EOF here is not an error.
        yield { text: js.substring(startIndex), type: 'scomment', start: startPos, end: pos.clone() }
        startIndex = i = len
        startPos = pos.clone()
        console.assert(startPos.position === startIndex && pos.position === i, `oops scomment-eof (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)

        break SCAN
      }
    } else if (js[i] === '\'') { // PL/SQL: double-quotes are identifiers, backticks are nonexistent
      ...
      while (i < len) {
        ...
        if (i < 0) {
          // EOF
          yield pos.enrichToken({ text: js.substring(startIndex), type: 'string', error: 'Unterminated string' });
          startIndex = i = len
          console.assert(startPos.position === startIndex && pos.position === i, `oops string-eof (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)
          break SCAN
        }
        ...
        yield pos.enrichToken({ text: js.substring(startIndex, i), type: 'string' })
        startIndex = i
        console.assert(startPos.position === startIndex && pos.position === i, `oops string (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)
        continue SCAN
      }
    }

    console.assert(startPos.position === startIndex && pos.position === i, 'oops before i++')
    if ('\n' == js[i]) {
      pos.newLine()
    } else {
      pos.increment()
    ...
    i++;
    console.assert(startPos.position === startIndex && pos.position === i, 'oops i++')
  }

  console.assert(startPos.position === startIndex && pos.position === i, `oops trailing i (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)

  // there is no trailing.
  // // trailing code. T
  // yield pos.enrichToken({ text: js.substring(start), type: 'code' });
  // i = len

  console.assert(startPos.position === startIndex && pos.position === i, `oops end (${pos} != ${JSON.stringify({ i, start: startIndex, len })})`)
...
exports.parseComments = function* (js, options) {
      ...
      case 'mcomment':
        ...
        if (prevTokens.length && prevComment) {
          ...
          for (const prevToken of prevTokens) {
            const trimText = exports.trimIndentation(prevToken.text).trim()
            prevComment.code.push(trimText);
            prevComment.ctx = exports.parseCodeContext(trimText, parentContext);
  ...
  if (prevTokens.length) {
    ...
    for (const prevToken of prevTokens) {
      const trimText = exports.trimIndentation(prevToken.text).trim()
      comment.code.push(trimText);
      comment.ctx = exports.parseCodeContext(trimText, parentContext);
