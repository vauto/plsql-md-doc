...
class DocumentGenerator {
  ...
  *parseSyntax(text) {
  ...
  isDocComment(token) {
    return token.type === 'comment.multi'
      && !token.text.startsWith('!') // not "/*!"
      && (!this.options.skipSingleStar || token.text.startsWith('*'))
  }
  ...
  *readComments(syntax) {
    ...
    const docComments = [...syntax.getLeadingTrivia()]
      .filter(token => this.isDocComment(token))
      .map(token => this.parseComment(token))
    console.log('comments', docComments)

    switch (syntax.kind) {
      case 'CreatePackageStatement':
        if (docComments.length === 0) {// && syntax.content.length > 0) {
          for (const child of syntax.children) {
            yield* this.readComments(child)
          }
          // Look inside the first content node.
          // docComments.push(...syntax.content[0].getLeadingTrivia())
        }
        break
      case 'EmptyStatement':
      case 'Identifier':
    ...
    for (const comment of docComments) {
      ...
      console.assert(comment instanceof Comment)
