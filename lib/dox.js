...
const { PlsqlItemName, PlsqlUnitName } = require('./plsql/name');
const {
  PlsqlNodeReader,
  ...
  OtherTypeDeclarationStatementSyntaxNode,
  ...
  SqlPlusStatementSyntaxNode,
  SqlStatementSyntaxNode,
  TerminatorSyntaxNode
...
class DocumentGenerator {
  ...
  *parseDocCommentSyntax(syntax) {
  ...
  contextPatternMatchers = [
     ...
     */
    (node, parentContext) => {
      if (node instanceof OtherTypeDeclarationStatementSyntaxNode) {
        return {
          parent: parentContext.codeContext,
          kind: 'type',
          name: new PlsqlItemName(node.name),
    ...
    (node, parentContext) => {
      if (node instanceof DeclarationParameterExpressionSyntaxNode) {
        ...
        const result = this.mapParameterToCodeContext(node, parent)

        switch (parent.kind) {
          case 'record':
            result.kind = 'field'
            return parent.fields.find(f => f.name === result.name) ?? result
          default:
    ...
    (node) => {
      if (node instanceof SqlPlusStatementSyntaxNode) { return undefined }
      if (node instanceof SqlStatementSyntaxNode) { return undefined }
      if (node instanceof TerminatorSyntaxNode) { return undefined }
      console.warn('nope!', node)
    }
