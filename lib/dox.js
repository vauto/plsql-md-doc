...
const { AnnotationNode, IdentifierSyntaxNode, SyntaxNode, mustBeNamedSyntaxNode } = require('./syntax')
const { Annotation, CodeContext, Comment } = require('./comment')
const { ArgumentValueError, mustBeInstanceOf } = require('./guards');
...
const {
  ...
  ReturnDeclarationExpressionSyntaxNodeBase,
  ...
  isMethodDeclarationSyntaxNode,
  MethodDeclarationExpressionSyntaxNodeBase,
  MethodDeclarationStatementSyntaxNode,
  CreateStandaloneMethodStatementSyntaxNode,
  CreatePackageBodyStatementSyntaxNode,
 ...
 * @typedef {import('./plsql/syntax').MethodDeclarationSyntaxNode} MethodDeclarationSyntaxNode
 */
...
class DocumentGenerator {
  ...
  *parseSyntax(text, { filename = null }) {
    // for (const t of this.plsqlLexer.parse(text, { filename })) {
  ...
  *readComments(syntax, parentContext) {
    ...
    switch (true) {
      ...
      case syntax instanceof CreatePackageBodyStatementSyntaxNode:
      ...
      case syntax instanceof CreateStandaloneMethodStatementSyntaxNode:
        break
      ...
      case syntax instanceof MethodDeclarationExpressionSyntaxNodeBase:
      case syntax instanceof MethodDeclarationStatementSyntaxNode:
  ...
  *parseDocCommentSyntax(syntax) {
  ...
  /**
   * @param {ReturnDeclarationExpressionSyntaxNodeBase?} node
   * @param {CodeContext} parent
   * @returns {CodeContext?}
   */
  ...
  /**
   * @param {MethodDeclarationSyntaxNode} node
   * @param {TokenFormat} format
   * @returns {string}
   */
  #formatMethodDeclaration(node, format = 'T') {
    if (node.parameterList) {
      const tokens = node.tokens
      const { openParenToken, closeParenToken } = node.parameterList
      const throughOpenParen = tokens.slice(0, tokens.indexOf(openParenToken))
      const closeToEnd = tokens.slice(tokens.indexOf(closeParenToken))
      ...
      return [
        `${stringifyTokenArray(throughOpenParen, format)}${openParenToken}`,
        node.parameters
  ...
  contextPatternMatchers = [
    ...
    (node, parentContext) => {
      if (node instanceof CreatePlsqlUnitStatementSyntaxNode) {
        const result = new CodeContext({
          ...
          header: node.createExpression.toString('T'),
        })
        result.annotations = this.mapAnnotationsToCodeContext(node.annotations, result)
        return result
      }
     ...
     */
    (node, parentContext) => {
      if (isMethodDeclarationSyntaxNode(node)) {
        const result = new CodeContext({
          parent: parentContext.codeContext,
          kind: node.memberKind.toLowerCase(),
          subkind: node.methodKind.toLowerCase(),
    ...
    (node, parentContext) => {
      if (node instanceof RecordTypeDeclarationStatementSyntaxNode) {
        const result = new CodeContext({
          parent: parentContext.codeContext,
          kind: 'type',
          subkind: this.#getTypeKindDescription(node.baseType),
          name: this.#mapIdentifier(node.name),
