...
const { PlsqlSyntaxNodeFactory } = require('./plsql/syntax');
const { JavadocSyntaxNodeFactory, TagSyntaxNode } = require('./javadoc/syntax');
...
const plsqlLexer = new PlsqlTokenizer()
const plsqlSyntax = new PlsqlSyntaxNodeFactory()
 ...
 */
exports.parseSyntax = function* (text, options) {
  const tokens = [...plsqlLexer.parse(text)]
  yield* plsqlSyntax.toSyntax(tokens)
};
...
exports.parseComments = function* (text, options) {
  options = options || {};
  text = text.replace(/\r\n/gm, '\n');
  ...
  for (const syntax of exports.parseSyntax(text, options)) {
    console.log(`plsql [${syntax.start}..${syntax.end}]`, JSON.stringify(syntax.toString().replace(/(?<=^.{20}).+/s, '...')), syntax)
    const docComments = [...syntax.getLeadingTrivia()]
      .filter(isDocComment)
      .map(token => exports.parseComment(token, options))

    if (prevSyntax && prevDocComments.length === 0 && docComments.length) {
      for (const comment of docComments) {
        console.log('match?', prevSyntax.toString(), comment)
        comment.code.push(prevSyntax)
        yield comment
      }
    }

    for (const comment of docComments) {
      comment.code.push(syntax)
      yield comment
    }
...
const javadocLexer = new JavadocTokenizer();
const javadocSyntax = new JavadocSyntaxNodeFactory()

exports.parseCommentSyntax = function* (text, options) {
  const tokens = [...javadocLexer.parse(text)]
  yield* javadocSyntax.toSyntax(tokens)
}
...
exports.parseComment = function (token, options) {

  const nodes = [...exports.parseCommentSyntax(token.text, options)]
  const comment = new Comment(nodes)
  //console.group('parseComment', `[${token.line}:${token.col}]`)//, token.type, JSON.stringify(token.text.replace(/(?=^.{20}).+/, '...')))
  for (const syntax of nodes) {
    if (syntax instanceof TagSyntaxNode) {
      console.log(`  doc [${syntax.start}..${syntax.end}]`, `@${syntax.tag}`, JSON.stringify(syntax.toString().replace(/(?<=^.{20}).+/s, '...')), syntax)
    } else {
      console.log(`  doc [${syntax.start}..${syntax.end}]`, 'UNK', JSON.stringify(syntax.toString().replace(/(?<=^.{20}).+/s, '...')), syntax)
    }
  }
  //console.groupEnd()
  ...
  // const lines = description.full.split('\n') // two newlines? Hmm.
  // description.summary = lines[0]
  // description.body = lines.slice(1).join('\n')//.join('\n\n');
