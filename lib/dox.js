...
class DocumentGenerator {
  ...
  *parseDocCommentSyntax(syntax) {
    console.info('docs', syntax.textSpan.toString('FULL'))
    for (const langToken of syntax.getDocumentComments()) {
      const docTokens = []

      console.groupCollapsed(`tokens [${langToken.textSpan.toString('LC')}]`)
      try {
        for (const docToken of this.javadocLexer.parse(langToken.text, langToken.start)) {
          docTokens.push(docToken)
          docToken.state = this.javadocLexer.state
          switch (docToken.type) {
            case 'newline':
            case 'whitespace':
            case 'star':
            case 'identifier':
            ...
            case 'brace.close':
            case 'text.content':
              console.info(docToken.start.toString('LC'), `[${docToken.state}] ✅`, docToken.type, JSON.stringify(docToken.text))
              break
            default:
              console.info(docToken.start.toString('LC'), `[${docToken.state}] ❓`, docToken.type, JSON.stringify(docToken.text))
              break
      ...
      for (const node of new JavadocNodeReader(docTokens)) {
        if (node instanceof TagSyntaxNode) {
          console.info('tag ', node.textSpan.toString('LC'), node)
        } else {
          console.info('node', node.textSpan.toString('LC'), node)
        }
        // switch (docToken.type) {
        //   case 'tag':
        //   case 'tag.inline.start':
        //   case 'tag.inline.end':
        //     console.info(docToken.type, docToken.start.toString(), docToken.text)
        //     break
        //   default:
        //     switch (docToken.value) {
        //       case '{':
        //       case '}':
        //         console.info(docToken.type, docToken.start.toString(), docToken.text)
        //         break
