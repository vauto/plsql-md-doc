...
class DocumentGenerator {
  ...
  *parseDocCommentSyntax(syntax) {
    console.groupCollapsed('docs', syntax.textSpan.toString('FULL'))
    try {
      for (const langToken of syntax.getDocumentComments()) {
        ...
        console.groupCollapsed(`nodes  [${langToken.textSpan.toString('LC')}]`)
        try {
          for (const docSyntax of new JavadocNodeReader(docTokens)) {
            if (docSyntax instanceof TagSyntaxNode) {
              console.info('tag ', docSyntax.textSpan.toString('LC'), docSyntax)
            } else {
              console.info('node', docSyntax.textSpan.toString('LC'), docSyntax)
            }
            yield docSyntax
          }
        } finally {
