...
exports.parseTokens = function* (js, options) {
  ...
  const emitPriorToken = (priorType) => {
    ...
    const token = new Token({ type: 'code', text: js.substring(start.position, end.position), start, end })
  ...
  const emitCurrentToken = (type, endIndex, error = undefined) => {
    ...
    const end = pos.clone()
    const token = new Token({ type, text, start, end, error })
    start = end
