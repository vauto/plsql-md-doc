...
exports.parseComment = function (token, options) {
  ...
  console.group('parseComment', `[${token.line}:${token.col}]`, token.type, token.text)
  for (const syntax of exports.parseCommentSyntax(token.text, options)) {
    if (syntax instanceof TagSyntaxNode) {
      ...
      console.log(`[${syntax.start}..${syntax.end}]`, JSON.stringify(syntax.toString().replace(/(?<=^.{20}).+/s, '...')), syntax)
    }
  }
  console.groupEnd()
