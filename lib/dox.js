...
class DocumentGenerator {
  ...
  *parseCommentSyntax(text) {
  ...
  parseComment(token, code) {
    ...
    const comment = new Comment({ code, token, nodes, context: this.parseCodeContext(code) })
  ...
  contextPatternMatchers = [
    ...
    (node, parentContext) => {
      switch (node.kind) {
        case 'ProcedureDeclarationStatment':
        case 'FunctionDeclarationStatment':
          return {
            ...
            returnClause: node.returnClause?.toString()
          }
      }
    },
