...
exports.parseComments = function* (js, options) {
  ...
  for (const syntax of exports.parseSyntax(js, options)) {
    const docComments = [...syntax.getLeadingTrivia()]
      .filter(isDocComment)
      .map(token => exports.parseComment(token.text, options))
...
exports.parseComment = function (text, options) {

  for (const syntax of exports.parseCommentSyntax(text, options)) {
  ...
  //  */
  // should yield "foo" and not "* foo".
  const str = text.replace(/^[ \t]*\* ?/gm, '').trim()
