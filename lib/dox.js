...
exports.parseComment = function (token, options) {
  ...
  for (const syntax of nodes) {
    if (syntax instanceof TagSyntaxNode) {
      console.log(`[${syntax.start}..${syntax.end}]`, `@${syntax.tag}`, JSON.stringify(syntax.toString().replace(/(?<=^.{20}).+/s, '...')), syntax)
    } else {
      console.log(`[${syntax.start}..${syntax.end}]`, 'UNK', JSON.stringify(syntax.toString().replace(/(?<=^.{20}).+/s, '...')), syntax)
    }
  }
  console.groupEnd()
