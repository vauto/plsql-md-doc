...
class DocumentGenerator {
  ...
  *parseDocCommentSyntax(syntax) {
  ...
  contextPatternMatchers = [
     ...
     */
    (node) => {
      if (node instanceof TypeDeclarationStatementSyntaxNode && node.specification instanceof RecordTypeExpressionSyntaxNode) {
        return {
          kind: 'record',
          name: new PlsqlItemName(node.name),
          specification: node.specification?.toString('T'),
          fields: node.specification.parameters.map(this.mapParameterToCodeContext) ?? [],
          signature: `(${node.specification.parameters.map(p => p.type.toString('T')).join(', ')})`,
          annotations: node.annotations.map(this.mapAnnotationToCodeContext)
        }
      }
    },
