...
exports.parseComments = function* (js, options) {
  ...
  const flushPrevTokens = (comment) => {
    ...
    for (const token of comment.code) {
      ...
      if (ctx && (ctx.type === 'prototype' || ctx.type === 'class')) {
        parentContext = ctx;
        console.log('PUSH context', parentContext)
      }
      // reasons to clear the namespace
      // new property/method in a different constructor
      else if (parentContext && (!ctx || !ctx.constructor || !parentContext.constructor || parentContext.constructor !== ctx.constructor)) {
...
exports.contextPatternMatchers = [
   ...
   */
  str => {
    const match = /^\s*(cursor|procedure|function)\s+([\w$]+)\s*/i.exec(str)
    if (match) {
      return {
