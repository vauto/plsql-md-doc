...
exports.parseSyntax = function* (js, options) {
  ...
  for (const token of plsqlLexer.parse(js)) {
    buffer.push(token)
...
exports.parseComment = function (token, options) {
  ...
  for (const syntax of nodes) {
    if (syntax instanceof TagSyntaxNode) {
      console.log(`[${syntax.start}..${syntax.end}]`, `@${syntax.name}`, JSON.stringify(syntax.toString().replace(/(?<=^.{20}).+/s, '...')), syntax)
    } else {
  ...
  //  */
  // should yield "foo" and not "* foo".
  const str = token.text.replace(/^[ \t]*\* ?/gm, '').trim()

  var comment = new Comment(token.text)
    , raw = options.raw
    , description = {}
    , tags = str.split(/\n\s*@/);
