...
const { CodeContext, Comment, Position, Span, SyntaxNode, SyntaxNodeFactory } = require('./model')
...
exports.parseComments = function* (js, options) {
   ...
   */
  const flushPrevTokens = (comment) => {
    for (const prevToken of prevTokens) {
      comment.code.push(prevToken)
    }
    prevTokens = []

    let /** @type {c} */ parentContext = parentContexts.at(-1)

    for (const token of comment.code) {
      const /** @type {CodeContext} */ ctx = comment.ctx = exports.parseCodeContext(token, parentContext);
      if (comment.isConstructor && ctx) {
      ...
      if (ctx) {
        switch (ctx.type) {
          case 'record':
            ...
            parentContexts.push(parentContext = ctx);
            console.log('PUSH context', parentContext)
            continue
        }
      }
      ...
      if (parentContext) {
        ...
        if (!ctx || !ctx.constructor || (ctx.type === 'end' && ctx.name === parentContext.name) || !parentContext.constructor || parentContext.constructor !== ctx.constructor) {
          console.log('POP context', parentContexts.pop())
          parentContext = null;
          continue
        }

        if (ctx) {
          parentContext.children.push(ctx)
          //console.log('child context', parentContext, ctx)
        }
      }
    }
  };
  ...
  const isDocComment = (token) => token.type === 'comment.multi'
    ...
    && (!skipSingleStar || token.text.startsWith('*'))

  let /** @type {SyntaxNode?} */ prevSyntax = null
  for (const syntax of exports.parseSyntax(js, options)) {
    ...
    // switch (syntax.type) {
    //   case 'mcomment':
    //     if (skipSingleStar && (syntax.text.length === 4 || syntax.text[2] !== '*')) {
