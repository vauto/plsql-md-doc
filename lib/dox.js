...
const { PlsqlTokenizer, JavadocTokenizer } = require('./lexer')
...
const jdLexer = new JavadocTokenizer();
...
exports.parseComment = function (text, options) {
  for (const token of jdLexer.parse(text)) {
    let /** @type {Token[]} */ buffer = []
    ...
    console.log(`[${token.line}:${token.col}]`, token)
  }
  ...
  console.group('javadoc', token.text.replace(/\n/g, '\\n'))
  console.groupEnd()
