...
dox.parseComments = function(js, options){
  ...
  while (i < len) {
    ...
    // console.assert(!withinString)

    if (!withinString && (js[i] === '/' && js[i+1] === '*')) {
    ...
    } else if (!withinString && '-' == js[i] && '-' == js[i+1]) {
      ...
      buf += strComment;
    } else if (!withinEscapeChar && ('\'' == js[i] || '"' == js[i] || '`' == js[i])) {
      if(withinString) {
