...
exports.parseCommentSyntax = function* (text, options) {
  ...
  let /** @type {Token[]} */ buffer = []
  for (const token of javadocLexer.parse(text)) {
...
exports.parseComment = function (token, options) {
  ...
  console.group('parseComment', `[${token.line}:${token.col}]`)
  for (const syntax of exports.parseCommentSyntax(token.text, options)) {
    console.log(`[${syntax.line}:${syntax.col}]`, syntax)
  }
  console.groupEnd()
  ...
  // console.log('parseComment', JSON.stringify(str))
