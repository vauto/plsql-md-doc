...
const { Position, Comment } = require('./model')
...
exports.parseTokens = function* (js, options) {
  ...
  let startIndex = 0, pos = new Position(), start = pos.clone()

  const assertOrDie = (message, text = null, log = true) => {
    if (log) {
      if (typeof text !== 'string') {
        text = js.substring(startIndex, i)
      }
      let wart = text.replace(/\n/g, "\\n")
      if (wart.length > 40) {
        wart = wart.substring(0,20) + '...' + wart.substring(wart.length-20)
      }
      console.log(message, start.toString(), '===', startIndex, '|', pos.toString(), '===', i, '|', wart)
    }

    if (start.position !== startIndex) {
      throw `${message}: start position mismatch: start: ${start} !== ${startIndex} | current: ${pos} ?== ${i} | len: ${len}`
    }
    if (pos.position !== i) {
      throw `${message}: current position mismatch: start: ${start} === ${startIndex} | current: ${pos} !== ${i} | len: ${len}`
    }
  }
  assertOrDie('start loop', null, false)

  const emitPriorToken = (priorType) => {
    assertOrDie(`emitting code prior to ${priorType}`, null, false)
    const end = pos.clone() // pos already == i here

    const token = { type: 'code', text: js.substring(start.position, end.position), start, end }

    startIndex = i
    start = end
    assertOrDie(`emitted code prior to ${priorType}`, token.text, false)
    return token
  }
  ...
  const emitCurrentToken = (type, endIndex, error = undefined) => {
    ...
    assertOrDie(`emitting ${type} 1${suffix}`, null, false)

    const text = js.substring(startIndex, endIndex)

    pos.advance(text)
    i = endIndex
    assertOrDie(`emitting ${type} 2${suffix}`, null, false)

    const end = pos.clone()
    const token = { text, type, start, end, error }
    startIndex = i
    start = end

    assertOrDie(`emitted ${type}${suffix}`, token.text, false)
    return token
  }
  
  SCAN:
  while (i < len) {
    assertOrDie('start iteration', null, false)
    if (js[i] === '/' && js[i + 1] === '*') {
      ...
      let endIndex = i + 1 // skip opening quote.
      while (endIndex < len) {
    ...
    assertOrDie('before i++', null, false)
    if ('\n' == js[i]) {
      pos.newLine()
    } else {
      pos.increment()
    ...
    i++;
    assertOrDie('after i++', null, false)
  }

  assertOrDie('trailing', null, false)
...
exports.parseComments = function* (js, options) {
      ...
      case 'mcomment':
        ...
        if (prevTokens.length && prevComment) {
          ...
          for (const prevToken of prevTokens) {
            prevComment.ctx = exports.parseCodeContext(prevToken, parentContext);
            prevComment.code.push(prevToken.trimText);
  ...
  if (prevTokens.length) {
    ...
    for (const prevToken of prevTokens) {
      comment.ctx = exports.parseCodeContext(prevToken, parentContext);
      comment.code.push(prevToken.trimText)
...
exports.parseCodeContext = function (token, parentContext) {
  console.log('parseCodeContext', token.start.toString('L') + '..' + token.end.toString('L'), token)
  token.trimText ??= exports.trimIndentation(token.text).trim()
  ...
  return exports.contextPatternMatchers.some(function (matcher) {
    return ctx = matcher(token.trimText, parentContext);
  }) && ctx;
};
