...
const { CodeContext, Comment, Position, Span, Token } = require('./model')
...
exports.parseComments = function* (js, options) {
  ...
  const flushPrevTokens = (comment) => {
    ...
    for (const token of comment.code) {
      ...
      if (parentContext && (!ctx || !ctx.constructor || !parentContext.constructor || parentContext.constructor !== ctx.constructor)) {
        console.log('POP context', parentContext)
        parentContext = null;
      }
    }
  };
...
exports.parseCodeContext = function (token, parentContext) {
  ...
  let /** @type {CodeContext?} */ ctx;

  if (parentContext.type) {
    console.log('has parent context', parentContext, token.trimText)
  }
...
/**
 * @typedef {Func<string, CodeContext?, CodeContext>} PatternMatcher
 * @type {PatternMatcher[]}
 */
exports.contextPatternMatchers = [
   ...
   */
  (str, parentContext) => {
    const match = /^\s*(?<type>cursor|procedure|function|type)\s+(?<name>[a-z][\w$#]*|"[^"]*")\s*/i.exec(str)
    if (match) {
      return {
   ...
   */
  (str, parentContext) => {
    console.log('variables', parentContext, str)
