 ...
 *
 * NOTE: There are so many properties because Handlebars will not accept prototype properties by default,
 * and the mechanism by which you whitelist them doesn't seem to work with getters.
 */
class Entity {
  /** @type {string} */ kind
  /** @type {string} */ subkind
  ...
  /** @type {string} The source file name for this entity. */ sourceFileName
...
class EntityGroup extends Array {
  ...
  push(...items) {
    ...
    this.kind = this.#aggregateKind(...this.map(i => i.kind))
    this.subkind = this.#aggregateKind(...this.map(i => i.subkind))
    return result
  }
  ...
  /**
   * @param  {...string} items
   * @returns {string[]} Unique kind list
   */
  #aggregateKind(...items) {
    switch (items.length) {
      case 0:
      ...
      default:
        ...
        const kinds = items.reduce((agg, item) => {
          if (item in agg) {
            agg[item]++
          } else {
            agg[item] = 1
        ...
        for (const [name, count] of Object.entries(kinds)) {
          result.push(this.#pluralize(name, count))
        }

        result.sort()
        console.assert(result.length, 'oops')
        return result
    }
  }
}
...
class UnitEntity extends Entity {
  ...
  /** @type {string} The output doc file for this unit. */ docFileName
