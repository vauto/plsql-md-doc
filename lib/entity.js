...
const { Description, CodeContext, Comment, LinkTag, IncludeTag, Annotation } = require('./comment.js');
...
class Entity {
  ...
  /** @type {CodeContext[]?} */ params
  /** @type {CodeContext?} */ return
  ...
  constructor(comment = undefined) {
    if (comment) {
      ...
      this.params = comment.context.params
      this.return = comment.context.return
      this.fields = comment.context.fields
      this.optional = comment.context.optional
      this.baseType = comment.context.baseType

      console.assert(comment, 'missing')
      console.assert(comment.context, 'missing')
      console.assert(comment.context.annotations, 'missing')
      console.assert(comment.context.annotations.every(a => a.name), 'missing a name')
      ...
      for (const k of Object.keys(comment.context)) {
        switch (k) {
          case 'annotations': continue // already handled
          case 'parent': continue // handled outside this
        }

        if (!(k in this) || this[k] !== comment.context[k]) {
          console.warnOnce(k, 'losing value', k, comment.context[k])
        }
      }
