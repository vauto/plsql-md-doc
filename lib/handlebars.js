...
// Path helpers
Handlebars.registerHelper('path-basename', (name) => path.basename(name))
Handlebars.registerHelper('path-extname', (name) => path.extname(name))
Handlebars.registerHelper('path-dirname', (name) => path.dirname(name))

// URL helpers
Handlebars.registerHelper('fileURLToPath', url => URL.fileURLToPath(url))
Handlebars.registerHelper('pathToFileURL', path => URL.pathToFileURL(path))


Handlebars.registerHelper('url-relative', (url, ...args) => {
  const options = args.pop()
  const baseURL = args.length ? args[0] : options.data.renderer.baseURL

  return relativize(url, baseURL)
})
