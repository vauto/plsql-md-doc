...
/**
 * @param {Entity | EntityGroup} entity
 * @param {RuntimeOptions} options
 * @returns {string}
 */
const createEntityTitle = (entity, options) => {
  const /** @type {ContentRenderer} */ renderer = options.data.renderer
  const name = renderer.render(entity.name)
  const kinds = formatKind(entity.subkind)
  return `${name} ${kinds}`
}

Handlebars.registerHelper('createEntityTitle', createEntityTitle)
 ...
 * @returns {string}
 */
const createEntityLink = (entity, options) => {
  const text = createEntityTitle(entity, options)
  return slugLink(text, options)
}
Handlebars.registerHelper('createEntityLink', createEntityLink)
