 ...
 */
const escapeTextForMarkdown = (value) => {
 ...
 */
Handlebars.escapeExpression = escapeTextForMarkdown
...
Handlebars.registerHelper('renderLink',
  ...
  function (link) {
    ...
    if (link instanceof LinkTag) {
      const content = renderContent(link.content)
      ...
      if (!(link.href instanceof URL)) {
        console.infoOnce(link.href, "Link to unresolved member", link.href)
        // Link cannot be resolved, just return the text
        return content
      }

      return `[${escapeTextForMarkdown(content)}](${link.href})`
    }
