...
Handlebars.registerHelper('renderLink', function (link) {
  if (!link) {
    // empty
    return ''
  }
  ...
  if (link instanceof LinkTag) {
    if (!link.href) {
      // Link is bare, just return the text
      return link.content
    }

    if (!(link.href instanceof URL)) {
      console.assert(false, 'handle this now')
    }
  ...
  console.assert(false, 'something else', link)
