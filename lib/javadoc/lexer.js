...
/**
 * @type {moo.Rule}
 * Default whitespace padding rule.
 * Leading whitespace and stars is considered separate from content-whitespace, and summarily ignored.
 * @see https://docs.oracle.com/javase/8/docs/technotes/tools/windows/javadoc.html#JSWOR648
 */
const paddingRule = {
  match: /^[ \t*]+/
}
...
class JavadocTokenizer extends Tokenizer {
  static #lexerData = {
    ...
    // For immediately after a code fence starts.
    'codeBlock': {
      newline: newlineRule,
      padding: /^[ \t]+\* /, // SPECIAL: we only discard up to the *first* space past the asterisk.
      whitespace: whitespaceRule,
      ...
      // Everything else in the block is text content
      'text.content': {
        match: /./,
        lineBreaks: true
      }
