const { SyntaxNode, SyntaxNodeOrToken, TokenLike, StatementSyntaxNode, SyntaxNodeReader, ExpressionSyntaxNode } = require('../syntax')
...
class JavadocNodeReader extends SyntaxNodeReader {
  ...
  *#readAsAllContentOrExpressions(token) {
  ...
  #readAsParamTag(token) {
    ...
    const name = this.#tryReadNextAsStrictContent()
    const eq = this.tryReadNextToken('=')
    const defaultValue = eq ? this.#tryReadNextAsStrictContent() : null

    return new ParamTagSyntaxNode(token, {
  ...
  readInternal() {
    while (!this.iterator.nextNonTrivial().done) {
      ...
      console.log('read', { type: token.type, value: token.value })
      ...
      switch (token.type) {
        ...
        default:
          console.assert(false, 'unexpected type', token.type, token)
          return new SyntaxNode(token)
      }
    }
  }
...
class ParamTagSyntaxNode extends TagSyntaxNode {
  constructor(kind, { type, name, eq, defaultValue }) {
