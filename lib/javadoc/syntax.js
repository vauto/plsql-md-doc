...
const { SyntaxNode, SyntaxNodeOrToken, SyntaxNodeFactory, TokenIterator, StatementSyntaxNode, SyntaxNodeReader } = require('../syntax')
...
class JavadocSyntaxNodeFactory extends SyntaxNodeFactory {
  ...
  #readAllContent(iterator) {
    if (!iterator || !iterator.is({ type: 'content' })) {
      throw new Error('not at content')
    }
...
class BracesSyntaxNode extends StatementSyntaxNode {
  lbrace
  expression
  rbrace
