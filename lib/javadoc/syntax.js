...
const { PlsqlMemberName } = require('../plsql/name')
const {
  ...
  *#readAsAllContentOrExpressions(token) {
  ...
  #tryReadNextAsBareReference() {
    ...
    if (lparen) {
      ...
      const rparen = parametersWithCommas.pop()
  ...
  #readAsParamTag(token) {
    ...
    defaultExpression = this.tryReadNextToken('=')
    defaultValue = defaultExpression ? this.#tryReadNextAsValueExpression() : null
      ...
      switch (token.type) {
        case 'tag':
          switch (token.value.toLowerCase()) {
            ...
            // unknown
            default:
              console.warnOnce(`[javadoc] ${token.textSpan} unknown tag ${token}`, token)
              return new TagSyntaxNode(token, {
                content: [...this.#tryReadNextAllContentOrExpressions()]
              })
          }
        ...
        default:
          console.info(`[javadoc] ${token.textSpan} unexpected token ${token.type}:${JSON.stringify(token.value)}`, token)
          return new DescriptionTagSyntaxNode(null, [...this.#readAsAllContentOrExpressions(token)])
      }
    }
  }
...
class TagSyntaxNode extends StatementSyntaxNode {
  ...
  constructor(tag, { name, content, ...params } = {}) {
    super()
    this.kind = tag.toString().toLowerCase()
    ...
    if (name) {
      this.push(name)
      this.name = PlsqlMemberName.from(name)
    }
