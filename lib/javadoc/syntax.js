...
const { ArgumentValueError, mustBeInstanceOf, mustBeObject, mustBeNonEmptyArray, ofType } = require('../guards')
const {
...
class JavadocNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsNamePathParametersWithCommas() {
  ...
  /**
   * @param {...TokenLike} tokens The initial tokens
   * @returns {UrlLiteralSyntaxNode}
   */
  #readAsUrlFragment(...tokens) {
    const result = [...tokens]
    let nextTokens
    while (nextTokens = this.tryReadNextTokens('-', Patterns.IDENTIFIER)) {
      result.push(...nextTokens)
    }

    return new UrlLiteralSyntaxNode(...result)
  }

  /**
   * Tries reading the next item as a name path, a URL, or a filename.
   *
   ...
   *
   * Filenames are similar except they typically contain a file extension.
   *
   * Absolute URLs are relatively easy to identify, but relative URLs overlap with filenames and name paths starting with `#`.
   * @returns {NamePathOrUrlLiteralSyntaxNode?}
   */
  #tryReadNextAsNamePathOrUrlOrFile() {
    // Try reading an identifier first.
    // If one exists, it's a NamePath.
    ...
    // Try reading '.' followed by an identifier.
    // This MAY be a complex identifier.
    const dotIdentifier = this.tryReadNextTokens(Patterns.PERIOD, Patterns.IDENTIFIER)
    if (dotIdentifier) {
      return new NamePathExpressionSyntaxNode(
        this.#readAsIdentifier(...dotIdentifier),
        this.#tryReadNextAsNamePathParameterList()
      )
    }

    // Try reading '#' followed by an identifier.
    // '#' doubles in meaning as "relative URL that is only a fragment" or "name path section";
    // however, if it starts with a hash, it MAY NOT be a compound identifier ('#' identifier-token).
    const hashIdentifier = this.tryReadNextTokens(Patterns.HASH, Patterns.IDENTIFIER)
    if (hashIdentifier) {
      const openParenToken = this.tryReadNextToken(Patterns.OPEN_PAREN)
      if (openParenToken) {
        // This is a NamePath expression starting with '#' (e.g., '#format_error_stack()')
        return new NamePathExpressionSyntaxNode(
          new IdentifierSyntaxNode(...hashIdentifier),
          this.#readAsNamePathParameterList(openParenToken)
        )
      }

      // This is part of a URL fragment.
      return this.#readAsUrlFragment(...hashIdentifier)
    }

    // Absolute URL or file path
    const urlToken = this.tryReadNextToken(Patterns.URL)
    return urlToken ? new UrlLiteralSyntaxNode(urlToken) : null
  }
  ...
  *#readNextAsBracketContents() {
  ...
  #readAsInlineContentLike(token) {
    ...
    switch (token.type) {
      ...
      case Patterns.NUMBER_LITERAL.type:
  ...
  *#readNextAllAsBlockContentLike() {
  ...
  #readAsThrowsExceptionTag(tagToken) {
    return new ThrowsExceptionTagSyntaxNode(
      tagToken,
      // Can be an identifier or even a number.
      this.#tryReadNextAsIdentifier() ??
      this.#tryReadNextAsLiteralExpression(),
      ...this.#readNextAllAsInlineContentLike())
  }
  ...
  #readAsTag(tagToken) {
    switch (tagToken.value.toLowerCase()) {
      ...
      case 'code':
        return new CodeTagSyntaxNode(tagToken, ...this.#readNextAllAsInlineContentLike())
...
/**
 * Tag with code content.
 */
class CodeTagSyntaxNode extends ContentTagSyntaxNode {
}
exports.CodeTagSyntaxNode = CodeTagSyntaxNode
...
class ParamTagSyntaxNode extends TagSyntaxNode {
  ...
  constructor(/** @type {TokenLike} */ tagToken, /** @type {SyntaxNodeOrTokenLike[]} */ ...params) {
    ...
    if (nameOrBracketExpression instanceof IdentifierSyntaxNode) {
      ...
      this.name = nameOrBracketExpression.expression.findDescendant(ofType(IdentifierSyntaxNode))
      // don't really care about the default value
      mustBeArrayOfContentLike(rest)
      this.content = rest
    }
    mustBeInstanceOf(this.name, IdentifierSyntaxNode, 'name')
  }
}
...
class ThrowsExceptionTagSyntaxNode extends TagSyntaxNode {
  /** @type {IdentifierSyntaxNode | LiteralSyntaxNode} */ name

  /**
   * @param {TokenLike} tagToken
   * @param {IdentifierSyntaxNode | LiteralSyntaxNode} name
   * @param {...ContentSyntaxNode} content
   */
  constructor(tagToken, name, ...content) {
    super('throws', tagToken, name, ...content)
    mustBeArrayOfContentLike(content, 'content')
    mustBeInstanceOf(name, SyntaxNode, 'name')
