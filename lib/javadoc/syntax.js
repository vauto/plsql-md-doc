...
class Patterns {
  ...
  static TEXT_OTHER = { type: 'text.other' }
  ...
  static CONTENT_LIKE = [
    ...
    // These signify things inline with content
    this.OPEN_BRACE,
    this.HTMLTAG_OPEN,
...
class JavadocNodeReader extends SyntaxNodeReader {
  ...
  * #readNextAsBracketContents() {
  ...
  #tryReadNextAsContentLike() {
    const token = this.tryReadNextToken(Patterns.CONTENT_LIKE)
  ...
  * #readNextAllAsContentLike() {
  ...
  #readAsMarkdownCodeExpression(token) {
    this.verify({ type: 'markdown.codeFence' }, token)
