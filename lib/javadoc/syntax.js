...
class JavadocNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsInvocationParametersWithCommas() {
   ...
   * @returns {NamePathOrUrlSyntaxNode?}
   */
  ...
  * #readUpTo(pattern) {
  ...
  #tryReadNextAsBracketContent() {
    return this.#tryReadNextAsTag() ?? this.#tryReadNextAsNamePathOrUrl()
  }
  ...
  *#readNextAllAsContentLike() {
  ...
  #readAsLinkTag(tagToken) {
    const namePathOrUrl = this.#tryReadNextAsNamePathOrUrl()
    if (!namePathOrUrl) {
      // this used to be linkExpression which will look for brace expression and bare reference.
      const oops = this.#tryReadNextAsBraceExpression()
      console.assert(!oops, 'oops brace expression')
    }
  ...
  #readAsThrowsExceptionTag(tagToken) {
    return new ThrowsExceptionTagSyntaxNode(tagToken, this.#tryReadNextAsNamePath(), ...this.#readNextAllAsContentLike())
  }
...
/**
 * @typedef {NamePathExpressionSyntaxNode | UrlSyntaxNode} NamePathOrUrlSyntaxNode
 */
...
exports.UrlSyntaxNode = UrlSyntaxNode
...
class LinkTagSyntaxNode extends TagSyntaxNode {
  /** @type {NamePathOrUrlSyntaxNode?} */ href
  /** @type {ContentSyntaxNode[]} */ content

  /**
   * @param {TokenLike} tagToken
   * @param {NamePathOrUrlSyntaxNode?} href
   * @param {...ContentSyntaxNode} content
   */
