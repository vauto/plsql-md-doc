...
class JavadocNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsNamePathParametersWithCommas() {
  ...
  #tryReadNextAsNamePathParameterList() {
    ...
    return openParenToken ? this.#readAsNamePathParameterList(openParenToken) : null
  }
  ...
  #tryReadNextAsNamePathOrUrlOrFile() {
    return this.#tryReadNextAsNamePath() ?? this.#tryReadNextAsUrl() ?? this.#tryReadNextAsFile()
  }
  ...
  *#readNextAllAsBlockContentLike() {
   ...
   */
  #tryReadNextAsFile() {
    const token = this.tryReadNextToken(Patterns.FILE)
    return token ? new UrlLiteralSyntaxNode(token) : null
  }
  ...
  #readAsIncludeTag(tagToken) {
    return new IncludeTagSyntaxNode(
      tagToken,
      this.#tryReadNextAsUrl() ?? this.#tryReadNextAsFile()
    )
  }
 ...
 * `@include` tag.
 */
class IncludeTagSyntaxNode extends TagSyntaxNode {
  /** @type {UrlLiteralSyntaxNode?} */ url

  /**
   * @param {TokenLike} tagToken
   * @param {UrlLiteralSyntaxNode?} url
   */
  constructor(tagToken, url) {
    super(tagToken, url)
    this.url = url
  }
}
