...
class JavadocNodeReader extends SyntaxNodeReader {
  ...
  #tryReadNextWhenBracesExpression() {
    const lbrace = this.tryReadNextTokenIf('{')
    if (!lbrace) {
      return null
    }
  ...
  #tryReadNextIfContent() {
    const content = this.tryReadNextTokenIf({ type: 'content' })
          ...
          switch (token.value) {
            ...
            case 'param':
              ...
              return new TagSyntaxNode(token, {
                ...
                description: this.#tryReadNextAllContent()
              })
            ...
            case 'returns':
              ...
              return new TagSyntaxNode(token, {
                type: this.#tryReadNextWhenBracesExpression(tokenIterator),
                description: this.#tryReadNextAllContent(tokenIterator)
              })

            case 'see':
              return new TagSyntaxNode(token, {
                expression: this.#tryReadNextWhenBracesExpression(tokenIterator),
                description: this.#tryReadNextAllContent(tokenIterator)
              })
            ...
            case 'type':
              return new TagSyntaxNode(token, {
                type: this.#tryReadNextIfContent(tokenIterator),
                description: this.#tryReadNextAllContent(tokenIterator)
              })
            case 'throws':
            case 'exception':
              return new TagSyntaxNode(token, {
                name: this.#tryReadNextIfContent(tokenIterator),
                description: this.#tryReadNextAllContent(tokenIterator)
              })
            case 'description':
