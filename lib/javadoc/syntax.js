...
class JavadocNodeReader extends SyntaxNodeReader {
  ...
  * #readNextAllAsContentLike() {
   ...
   */
  #readAsHtmlTag(token) {
    return new ContentSyntaxNode(token, ...this.readNextTokensUntil(Patterns.ANY_HTML_CLOSE_TAG))
  }

  /**
   * Reads a markdown code expression.
  ...
  #readAsMarkdownCodeExpression(token) {
    this.verify({ type: 'markdown.code.open' }, token)
    return new ContentSyntaxNode(token, ...this.readNextTokensUntil({ type: 'markdown.code.close', value: token.value }))
  }
