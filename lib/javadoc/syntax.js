...
class JavadocNodeReader extends SyntaxNodeReader {
  ...
  * #readNextAsBracketContents() {
   ...
   */
  #readAsContent(token) {
    this.verify(Patterns.CONTENT, token)
    return new ContentSyntaxNode(token, ...this.readNextTokensWhile(Patterns.CONTENT))
  }
  ...
  #tryReadNextAsContent() {
    const token = this.tryReadNextToken(Patterns.CONTENT)
  ...
  #readAsContentLike(token) {
    ...
    console.assert(this.matches(Patterns.CONTENT, token), `${token.start} Token type is not content: '${token.type}'`, token)
  ...
  *#readNextAllAsContentLike() {
  ...
  /**
   * Reads from the current token as an HTML tag (e.g., `<p>, `<a href=...>`)
...
class SeeTagSyntaxNode extends TagSyntaxNode {
  ...
  /** @type {SyntaxNodeOrToken[]} */ content
 ...
 *
 * @param {any} value
 * @returns {value is ContentLikeSyntaxNode}
 */
const isContentLike = (value) => value instanceof ContentSyntaxNode || value instanceof UrlSyntaxNode || value instanceof BraceExpressionSyntaxNode

/**
 * @param {any} value
 * @param {string} paramName
 * @returns {asserts node is ContentLikeSyntaxNode}
 */
 ...
 * @param {any[]} array
 * @param {string} paramName
 * @returns {asserts array is ContentLikeSyntaxNode[]}
 */
