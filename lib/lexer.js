...
class Tokenizer {
  ...
  /**
   * Calculates {@link Token.isTrivia} from the token type.
   * @protected
  ...
  calculateIsTrivia(tokenType) {
    switch (tokenType) {
      ...
      case 'comment.doc':
        return 'structured'
      default:
        return tokenType.startsWith('comment.')
    }
  }
  ...
  #decorate(mooToken, { filename }) {
    const isTrivia = this.calculateIsTrivia(mooToken.type)
    const textSpan = this.#calculateTextSpan(mooToken, filename)
    const token = new Token(mooToken, isTrivia, textSpan)
    return token
  }
   ...
   * @yields {Token}
   */
  *parse(input, { filename, line, col } = { filename: null, line: null, col: null }) {
