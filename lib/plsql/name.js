...
class PlsqlIdentifier extends Identifier {
  ...
  static #lexer = new PlsqlTokenizer()

  /**
   * Parses an identifier via a lexer.
   * @param {string} text
   * @returns {PlsqlIdentifier}
   */
  static parse(text) {
    const tokens = [...PlsqlIdentifier.#lexer.parse(text)]
    if (tokens.length !== 1 && tokens[0].type !== 'identifier') {
      throw new ArgumentValueError("text", text, "Value is not a valid PL/SQL identifier")
    }

    return new PlsqlIdentifier(tokens[0])
  }
}
