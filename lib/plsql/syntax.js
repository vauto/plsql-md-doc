...
class Patterns {
  ...
  static BEGIN = Patterns.reserved('BEGIN')
...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readThroughEndOfLine() {
  ...
  #readAsOpaquePlsqlStatement(token) {
    return new StatementSyntaxNode(token, ...this.readNextTokensUntil(Patterns.SEMICOLON))
  }
  ...
  *#readNextAsInvocationParametersWithCommas() {
  ...
  #tryReadNextAsDeclaration() {
    ...
    switch (token.value) {
      ...
      case 'CURSOR':
        throw this.notImplemented('cursor')
      default:
  ...
  *#readAllDeclarations() {
  ...
  /**
   * Tries reading the next item as a declare section.
   * @returns {DeclareSectionSyntaxNode?}
   */
  #tryReadNextAsDeclareSection() {
    const declaration = this.#tryReadNextAsDeclaration()
    return declaration ? new DeclareSectionSyntaxNode(declaration, ...this.#readAllDeclarations()) : null
  }
   ...
   */
  #readAsBlock(beginToken) {
    const /** @type {StatementSyntaxNode[]} */ statements = []
    let token
    while (token = this.readNextToken()) {
      switch (token.value) {
        case 'BEGIN':
          // Nested block.
          statements.push(new StatementSyntaxNode(this.#readAsBlock(token), this.#tryReadNextAsSemicolonToken()))
          break
        case 'END':
          return new BlockSyntaxNode(beginToken, statements, token, this.#tryReadNextAsIdentifier())
        default:
          // Not parsing these. (yet?)
          statements.push(this.#readAsOpaquePlsqlStatement(token))
          break
      }
    }

    // Malformed
    return new BlockSyntaxNode(beginToken, statements, null, null)
  }
   ...
   * @returns {BlockSyntaxNode}
   */
  #readAsEndOfBlock(endToken) {
    Token.mustMatch(Patterns.END, endToken)
    return new BlockSyntaxNode(null, null, endToken, this.#tryReadNextAsIdentifier());
  }
   ...
   */
  #tryReadNextAsBlock() {
    return this.tryReadNextAs(this.#readAsBlock, Patterns.BEGIN)
  }
  ...
  #tryReadNextAsMethodBody() {
    const declareSection = this.#tryReadNextAsDeclareSection()
    const block = this.#tryReadNextAsBlock()
  ...
  #readAsMethodDeclaration(methodKeyword) {
    ...
    const body = isOrAsKeyword ? this.#tryReadNextAsAnyMethodBody() : null
  ...
  #readRestOfStandaloneMethod(create, editionable, unitType, ifNotExists) {
    return new CreateStandaloneMethodStatementSyntaxNode(
      ...
      this.#tryReadNextAsAnyMethodBody(),
      this.#tryReadNextAsSemicolonToken(),
...
class MethodDeclarationStatementSyntaxNode extends MethodDeclarationStatementSyntaxNodeBase {
  ...
  /** @type {SyntaxNode?} The method body (optional) */ body
...
class BlockSyntaxNode extends ExpressionSyntaxNode {
   ...
   */
  constructor(beginToken, statements, endToken, name) {
    super(beginToken, statements, endToken, name)
...
class MethodBodySyntaxNode extends ExpressionSyntaxNode {
  /** @type {DeclareSectionSyntaxNode?} */ declareSection
  /** @type {BlockSyntaxNode} */ block

  /**
   * @param {DeclareSectionSyntaxNode?} declareSection
   * @param {BlockSyntaxNode} block
   */
