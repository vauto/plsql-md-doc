...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#tryReadNextAsParameterDeclarationsWithCommas() {
  ...
  #readAsFunctionDeclaration(token) {
    ...
    return new FunctionDeclarationStatementSyntaxNode(token, {
      name,
      parameters,
      return: returnClause,
      rest
  ...
  #readAsProcedureDeclaration(token) {
    ...
    const parameters = this.#tryReadNextAsParameterListDeclaration()
    const rest = this.readNextTokensUntil(';')
    return new ProcedureDeclarationStatementSyntaxNode(token, {
      name,
      parameters,
      rest
    })
...
class ProcedureDeclarationStatementSyntaxNode extends StatementSyntaxNode {
   ...
   * @property {SyntaxNode?} rest
   */
  constructor(token, { name, parameters, rest }) {
    super()
    this.push(token)
    this.add({ name, parameters, rest })
  }
}
...
class FunctionDeclarationStatementSyntaxNode extends StatementSyntaxNode {
   ...
   * @param {SyntaxNode} return
   * @param {SyntaxNode?} rest
   */
  constructor(token, { name, parameters, return: returnClause, rest }) {
    super()
    this.push(token)
    this.add({ name, parameters, return: returnClause, rest })
  }
}
