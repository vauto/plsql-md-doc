...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsTypeModifiers() {
  ...
  /**
   * Tries reading the next item as a type precision and scale expression, with enclosing parentheses.
   * @returns {ExpressionSyntaxNode?}
   */
  ...
  #readAsTypeExpression(token) {
    ...
    return notNull ? new RestrictedTypeExpressionSyntaxNode({ type, constraints: [notNull] }) : type
  }
 ...
 */
class RestrictedTypeExpressionSyntaxNode extends ExpressionSyntaxNode {

  /** @type {ExpressionSyntaxNode} The unrestricted type expression */ expression
  /** @type {SyntaxNode[]} The constraints. */ constraints

  constructor({ expression, constraints }) {
    super(expression, constraint)
    this.expression = expression
