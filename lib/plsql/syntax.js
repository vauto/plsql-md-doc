...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsTypeModifiers() {
   ...
   *
   */
  #tryReadNextAsJsonContent() {
    throw this.notImplemented()
  }

  /**
   * @param {TokenLike} jsonObjectToken `JSON_OBJECT`
   * @see https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/JSON_OBJECT.html
   * ```bnf
   * JSON_OBJECT "(" JSON_OBJECT_content ")"
   * ```
   */
  #readAsJsonObjectExpression(jsonObjectToken) {
    Token.mustMatch('JSON_OBJECT', jsonObjectToken)

    // NOTE: Because JSON_OBJECT is a *keyword*, not a *reserved* word, it is possible for it to be used as a regular identifier/invocation of something.
    // However, if we see it is followed by paren, parse it as if it is the keyword.
    const openParenToken = this.#tryReadNextAsOpenParenToken()
    return openParenToken
      ? new ExpressionSyntaxNode(jsonObjectToken, openParenToken, this.#tryReadNextAsJsonContent(), this.#readNextAsCloseParenToken())
      : this.#readAsInvocation(jsonObjectToken)
  }

  /**
   * @param {TokenLike} jsonToken `JSON`
   * @see https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/JSON_OBJECT.html
   * ```bnf
   * JSON "{" JSON_OBJECT_content "}"
   * ```
   */
  #readAsJsonExpression(jsonToken) {
    Token.mustMatch('JSON', jsonToken)

    // NOTE: Because JSON_OBJECT is a *keyword*, not a *reserved* word, it is possible for it to be used as a regular identifier/invocation of something.
    // However, if we see it is followed by paren, parse it as if it is the keyword.
    const openBraceToken = this.tryReadNextToken('{')
    return openBraceToken
      ? new ExpressionSyntaxNode(jsonToken, openBraceToken, this.#tryReadNextAsJsonContent(), this.readNextToken('}'))
      : this.#readAsInvocation(jsonToken)
  }
      ...
      switch (token.type) {
        ...
        case 'keyword':
          switch (token.value) {
            ...
            case 'JSON':
              return this.#readAsJsonExpression(token)
            case 'JSON_OBJECT':
              return this.#readAsJsonObjectExpression(token)
