...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readThroughEndOfLine() {
  ...
  #readAsSqlStatement(token, ...params) {
    return new SqlStatementSyntaxNode(token, ...params, this.#readNextAsSqlTerminator())
  }
  ...
  #readNextAsSqlStatement(...params) {
    return new SqlStatementSyntaxNode(...params, this.#readNextAsSqlTerminator())
  }
  ...
  *#readNextAsAccessorsWithCommas() {
  ...
  #tryReadNextAsAccessibleByClause() {
    ...
    return new AccessibleByExpressionSyntaxNode({
  ...
  *#readNextAsTypeModifiers() {
  ...
  /**
   * Tries reading the next item as a type precision expression, with enclosing parentheses.
   * @returns {ExpressionSyntaxNode?}
   */
  #tryReadNextAsPrecisionRestriction() {
    const lparen = this.#tryReadNextAsLeftParen()
    if (lparen) {
  ...
  /**
   * Reads starting with the given token as an integer type expression.
   * @example
   * ```sql
   * BINARY_INTEGER
  ...
  #readAsIntervalTypeExpression(name) {
    ...
    if (day) {
      return new ExpressionSyntaxNode({
        name,
        day,
        dayPrecision: this.#tryReadNextAsPrecisionRestriction(),
        to: this.readNextToken('TO'),
        second: this.readNextToken('SECOND'),
        secondPrecision: this.#tryReadNextAsPrecisionRestriction()
      })
    }
    ...
    if (year) {
      return new ExpressionSyntaxNode({
        name,
        year,
        yearPrecision: this.#tryReadNextAsPrecisionRestriction(),
        to: this.readNextToken('TO'),
  ...
  /**
   * Reads starting with the given token as a timestamp type expression.
   * ```text
  ...
  #readAsTimestampTypeExpression(name) {
    ...
    const precision = this.#tryReadNextAsPrecisionRestriction()
  ...
  /**
   * Reads starting with the given token as an unrestricted type expression.
   * @param {TokenLike} name The initial name token for the type
   * @returns {ExpressionSyntaxNode | IdentifierSyntaxNode}
   * @see {@link #readAsTypeExpression}
   */
  #readAsUnrestrictedTypeExpression(name) {
    ...
    switch (name.value) {
      ...
      case 'VARCHAR':
      case 'VARCHAR2': {
        return new CharacterTypeExpressionSyntaxNode(name, this.#tryReadNextAsLengthTypeRestriction())
      }
      case 'DECIMAL':
      case 'INTEGER': // nonrangeable, can specify precision/scale
      case 'NUMBER':
        return new NumericTypeExpressionSyntaxNode(name, this.#tryReadNextAsInvocationParameterList())
   ...
   * @param {TokenLike} token The initial token for the type
   * @returns {ExpressionSyntaxNode | IdentifierSyntaxNode}
   * @see {@link #readAsTypeExpression}
   */
  #readAsTypeExpression(token) {
    const type = this.#readAsUnrestrictedTypeExpression(token)
    const notNull = SyntaxNode.asSyntaxNode(this.tryReadNextTokens('NOT', 'NULL'))
    return notNull ? new OldTypeExpressionSyntaxNode(type, [notNull]) : type
  }
   ...
   * @returns {ExpressionSyntaxNode}
   */
  #readAsIntervalLiteralExpression(name) {
    this.verify(Patterns.INTERVAL, name)
   ...
   * @returns {ExpressionSyntaxNode}
   */
  #readAsTimestampLiteralExpression(name) {
    this.verify(Patterns.TIMESTAMP, name)
  ...
  *#readNextAsParameterDeclarationsWithCommas() {
  ...
  // -----------------------------------
  // Other SQL and PL/SQL statements
  // -----------------------------------
...
class AccessibleByExpressionSyntaxNode extends ExpressionSyntaxNode {
  ...
  constructor({ accessibleBy, accessors }) {
    super(accessibleBy, accessors)
    this.accessibleBy = unitKind
...
class IntegerTypeExpressionSyntaxNode extends TypeExpressionSyntaxNode {
  ...
  /**
   * @param {IdentifierSyntaxNode} name
   * @param  {RangeTypeRestrictionExpressionSyntaxNode?} range
   */
  ...
  get range() {
    return this.getFirstRestrictionByKind('range')
  }
}

/**
 * Represents a type with zero or more restrictions (e.g., `NOT NULL`).
 */
class OldTypeExpressionSyntaxNode extends ExpressionSyntaxNode {
  /** @type {ExpressionSyntaxNode | IdentifierSyntaxNode} The unrestricted type expression */ expression
  /** @type {InvocationParameterListExpressionSyntaxNode?} The restrictions. */ restrictions

  /**
   * @param {ExpressionSyntaxNode | IdentifierSyntaxNode} expression
   * @param {InvocationParameterListExpressionSyntaxNode?} restrictions
   * @param  {...SyntaxNodeOrToken} params
   */
  constructor(expression, restrictions, ...params) {
    super(expression, restrictions, ...params)
    this.expression = expression
    this.restrictions = restrictions?.restrictions
  }
}

class OldNamedTypeExpressionSyntaxNode extends OldTypeExpressionSyntaxNode {
   ...
   * @param  {...SyntaxNodeOrToken} params
   */
  constructor(name, restrictions, ...params) {
    super(name, restrictions, ...params)
  }
...
class CharacterTypeExpressionSyntaxNode extends TypeExpressionSyntaxNode {
  ...
  /**
   * @param {IdentifierSyntaxNode} name The type name (e.g., `VARCHAR2`).
   * @param {LengthTypeRestrictionExpressionSyntaxNode?} length The length restriction
   */
  ...
  get length() {
    return this.getFirstRestrictionByKind('length')
  }
}
exports.CharacterTypeExpressionSyntaxNode = CharacterTypeExpressionSyntaxNode

/**
 * Number type expression with optional length.
 * @example `NUMBER`
 * @example `NUMBER(5)`
 * @example `NUMBER(*, 0)`
 * @example `NUMBER(5, 2)`
 */
class NumericTypeExpressionSyntaxNode extends OldNamedTypeExpressionSyntaxNode {

  /** @type {ExpressionSyntaxNode?} */
  get precision() {
    return this.restrictions?.at(0)
  }

  /** @type {ExpressionSyntaxNode?} */
  get scale() {
    return this.restrictions?.at(1)
  }
}
exports.NumericTypeExpressionSyntaxNode = NumericTypeExpressionSyntaxNode
