...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsInvocationParametersWithCommas() {
  ...
  /**
   * Reads starting with the given token as a (compound) identifier that MAY be
   * part of an invocation of a procedure, function, cursor, method, pseudofunction, or similar.
  ...
  #readAsIdentifierOrInvocation(token) {
    ...
    const params = this.#tryReadNextAsInvocationParameterList()
    return params ? new InvocationExpressionSyntaxNode(identifier, params) : identifier
  }
  ...
  *#readNextAsPackageSpecContent() {
  ...
  #readRestOfPackageSpec(create, editionable, unitType) {
    ...
    const authid = this.#tryReadNextAsInvokerRightsClause()
  ...
  *#readNextAsTypeModifiers() {
      ...
      switch (token.type) {
        ...
        case 'keyword':
          switch (token.value) {
            ...
            // keywords that MAY be function calls + those that are.
            case 'CURRENT_DATE':
  ...
  #tryReadNextAsReturnModifier() {
    ...
    const modifier = this.#tryReadNextAsInvokerRightsClause()
...
class FunctionDeclarationStatementSyntaxNode extends MethodDeclarationStatementSyntaxNodeBase {
   ...
   * @param {SyntaxNode?} params.terminator
   */
  constructor({ keyword, name, parameters, returnClause, body, terminator }) {
    super(keyword, name, parameters, returnClause, body, terminator)
    this.keyword = keyword
    this.name = name
    this.parameters = parameters
...
class InvocationExpressionSyntaxNode extends ExpressionSyntaxNode {
   ...
   * @param {InvocationParameterListExpressionSyntaxNode} parameters
   */
