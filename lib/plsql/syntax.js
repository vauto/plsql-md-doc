...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsTypeModifiers() {
  ...
  #readAsTimestampTypeExpression(name) {
    ...
    const precision = this.#tryReadNextAsTypePrecisionExpression()
  ...
  #readAsUnrestrictedTypeExpression(name) {
    ...
    switch (name.value) {
      ...
      case 'VARCHAR':
      case 'VARCHAR2': {
        return new CharacterTypeExpressionSyntaxNode(name, this.#tryReadNextAsInvocationParameterList())
      }
      case 'DECIMAL':
 ...
 */
class CharacterTypeExpressionSyntaxNode extends OldNamedTypeExpressionSyntaxNode {
