...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#tryReadUpToNextDocComment() {
  ...
  #readNextAsPlsqlUnitAsIsKeyword() {
    ...
    const token = this.readNextToken(Patterns.IS_OR_AS)
    return new SyntaxNode(token, this.#tryReadUpToNextDocComment())
  }
  ...
  * #tryReadNextAsParameterDeclarationsWithCommas() {
  ...
  #readAsVariableDeclaration(token) {
    ...
    if (exception) {
      ...
      return new ExceptionDeclarationStatementSyntaxNode(name, exception, this.#readNextAsTerminator())
    }
