...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextTokens(count) {
  ...
  #readNextAsPackageSpec() {
    throw this.#notImplemented('Package spec')
    // <identifier>
    const name = this.readNextWhenIdentifier()

    // AUTHID [DEFINER|CURRENT_USER]
    let authid = null
    if (next = this.tryReadNextTokensIf('AUTHID', { type: 'identifier' })) {
      authid = new SyntaxNode(next)
    }

    // ACCESSIBLE BY (...)
    let accessibleBy = null
    if (next = this.tryReadNextTokensIf('ACCESSIBLE', 'BY', '(')) {
      // LATER: differentiate type, identifier
      accessibleBy = new SyntaxNode(...next, ...this.readNextTokensUntil(')'))
    }
  ...
  #readAsCreateStatement(token) {
    ...
    switch (objectType.value) {
      case 'FUNCTION':
      case 'PROCEDURE':
        return new PlsqlCreateStatementSyntaxNode(create, objectType, this.#readNextAsProcedure())
      case 'PACKAGE':
        return new PlsqlCreateStatementSyntaxNode(create, objectType, this.#readNextAsPackageSpec())
      case 'TYPE':
        return new PlsqlCreateStatementSyntaxNode(create, objectType, this.#readNextAsTypeSpec())
      default:
