...
class Patterns {
  ...
  static AS = { type: 'reserved', value: 'OF' }
...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#tryReadNextAsInheritanceClauses() {
  ...
  #tryReadNextAsObjectConstructorMethod(inheritance) {
    ...
    return new ConstructorDeclarationStatementSyntaxNode({
      ...
      return: new ConstructorReturnDeclarationExpressionSyntaxNode({
  ...
  *#tryReadNextAsTypeModifiers() {
  ...
  #readNextAsTypeSpec(create, editionable, unitType) {
    ...
    throw this.#notImplemented('unknown TYPE type')
  ...
  #tryReadNextAsTypeLengthExpression() {
    const lparen = this.#tryReadNextAsLeftParen()
    if (lparen) {
      return new ExpressionSyntaxNode({
        lparen,
  ...
  #readAsTimestampTypeExpression(name) {
    ...
    const length = this.#tryReadNextAsTypeLengthExpression()
      ...
      case 'keyword':
        switch (name.value) {
          ...
          case 'VARCHAR2': {
            return new ExpressionSyntaxNode({
              name,
              length: this.#tryReadNextAsTypeLengthExpression()
            })
          }
          case 'NUMBER': {
            return new ExpressionSyntaxNode({
        ...
        case 'operator':
          switch (token.value) {
            ...
            case '@@':
              ...
              return this.#readSqlPlusCommand(token)
          }
          break
      }
...
class CreatePackageStatementSyntaxNode extends CreatePlsqlUnitStatementSyntaxNode {
   ...
   * @param {SyntaxNode[]} content
   */
  constructor({ create, editionable, unitType, name, authid, accessibleBy, is, content }) {
    super()
    this.add({ create, editionable, unitType, name, authid, accessibleBy, is })
    this.push(content)
...
class CreateBaseObjectTypeStatementSyntaxNode extends CreateTypeStatementSyntaxNode {
   ...
   */
  constructor({ create, editionable, unitType, name, authid, accessibleBy, is, type, members, modifiers, rest }) {
    super()
    this.add({ create, editionable, unitType, name, authid, accessibleBy, type, is })
    this.push(members)
    ...
    for (const constructor of this.methods.filter(m => m instanceof ConstructorDeclarationStatementSyntaxNode)) {
      constructor.return.type ??= this.name
    }
  }
...
class CreateNestedTableTypeStatementSyntaxNode extends CreateTypeStatementSyntaxNode {
   ...
   * @param {SyntaxNode} params.rest
   */
  constructor({ create, editionable, unitType, name, authid, accessibleBy, is, specification, modifiers, rest }) {
    super()
    this.add({ create, editionable, unitType, name, authid, accessibleBy, is, specification, modifiers })
    this.push(rest)
  }
}
exports.CreateBaseObjectTypeStatementSyntaxNode = CreateBaseObjectTypeStatementSyntaxNode
 ...
 */
class ConstructorDeclarationStatementSyntaxNode extends MethodDeclarationStatementSyntaxNode {
}
