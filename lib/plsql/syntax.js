...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readThroughEndOfLine() {
  ...
  #readAsCreateStatement(createKeyword) {
    ...
    const unitType = this.#tryReadNextAsUnitType() ?? this.#tryReadNextAsSubstitutionVariableExpression()
  ...
  #tryReadNextAsUnitType() {
    ...
    } else if (next = this.tryReadNextTokens('GLOBAL', 'TEMPORARY', 'TABLE')) {
      return new UnitTypeSyntaxNode(next, next[2].value)
    }

    // General case
    next = this.tryReadNextToken(Patterns.ANY_KEYWORD)
    return next ? new UnitTypeSyntaxNode(next) : null
  }
