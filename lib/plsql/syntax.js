const { SyntaxNode, SyntaxNodeReader, ExpressionSyntaxNode, IdentifierSyntaxNode, StatementSyntaxNode, TokenIterator, TokenPattern } = require('../syntax')
...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#tryReadUpToNextDocComment() {
  ...
  #readNextAsValueExpression() {
    const token = this.readNextToken([{ type: 'string' }, { type: 'number' }])
  ...
  #tryReadNextAsParameterDeclaration() {
    ...
    return new SyntaxNode(lparen, ...this.readNextTokensUntil(')'))
  }
  ...
  #readAsFunctionDeclaration(token) {
    ...
    const name = this.readNextAsIdentifier()
    const parameters = this.#tryReadNextAsParameterDeclaration()
...
class ProcedureDeclarationStatementSyntaxNode extends TypeDeclarationStatementSyntaxNode {
