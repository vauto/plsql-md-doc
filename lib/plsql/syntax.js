...
const { mustBeInstanceOf, InvalidNumberOfArgumentsError, mustBeArray, mustBeObject, mustBeNonEmptyString } = require('../guards')
...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readThroughEndOfLine() {
  ...
  #readNextAsUnitType() {
    ...
    if (next = this.tryReadNextTokens(['TYPE', 'PACKAGE'], 'BODY')) {
      ...
      return new UnitTypeSyntaxNode(next, next.join(' '))
    }
  ...
  *#readNextAsInvocationParametersWithCommas() {
  ...
  #readAsTypeDeclaration(typeKeyword) {
    ...
    if (typeExpression instanceof RecordTypeExpressionSyntaxNode) {
  ...
  *#readNextAsTypeModifiers() {
  ...
  #readRestOfTypeSpec(create, editionable, unitType, ifNotExists) {
    ...
    throw this.notImplemented(this.iterator.value ?? isOrAsKeyword, 'unknown TYPE type')
  }
  ...
  #readAsIntervalLiteralExpression(intervalKeyword) {
    this.verify(Patterns.INTERVAL, intervalKeyword)
...
class CreatePackageStatementSyntaxNode extends CreatePlsqlUnitStatementSyntaxNode {
  ...
  constructor(header, isOrAsKeyword, docComment, content, semicolonToken, slashToken) {
    ...
    this.members = this.content.filter(/** @returns {x is DeclarationSyntaxNode} */ x => x instanceof DeclarationStatementSyntaxNode || x instanceof DeclarationExpressionSyntaxNode)
...
class CreateObjectTypeStatementSyntaxNode extends CreateTypeStatementSyntaxNode {
  ...
  get constructors() {
    return this.methods.filter(/** @returns {x is ConstructorDeclarationStatementSyntaxNode} */ x => x instanceof ConstructorDeclarationStatementSyntaxNode)
  }
...
class VariableDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
  ...
  constructor(name, constantKeyword, type, defaultToken, defaultValue, semicolonToken) {
    super(name, constantKeyword, type, defaultToken, defaultValue, semicolonToken)
...
class MethodDeclarationStatementSyntaxNodeBase extends DeclarationStatementSyntaxNode {
  ...
  constructor(...nodesOrTokens) {
    ...
    this.parameterList = this.children.find(/** @return {x is DeclarationParameterListExpressionSyntaxNode} */ x => x instanceof DeclarationParameterListExpressionSyntaxNode)
  }
...
class ProcedureDeclarationStatementSyntaxNode extends MethodDeclarationStatementSyntaxNodeBase {
  /**
   * @param {TokenLike} methodKeyword  The initial token (`PROCEDURE`)
   * @param {IdentifierSyntaxNode} name
   * @param {DeclarationParameterListExpressionSyntaxNode?} parameterList
   * @param {SyntaxNode[]} unitModifiers
   * @param {ExpressionSyntaxNode?} body An optional body.
...
class ConstructorDeclarationStatementSyntaxNode extends ObjectMethodDeclarationStatementSyntaxNodeBase {
  /** @type {ConstructorReturnDeclarationExpressionSyntaxNode} */ returnClause
...
class DeclarationParameterExpressionSyntaxNode extends DeclarationExpressionSyntaxNode {
  ...
  constructor(name, mode, type, defaultToken, defaultValue) {
    super(name, mode, type, defaultToken, defaultValue)
    this.name = name
    this.mode = mode
    this.type = type
    this.defaultToken = this.resolveToken(defaultToken)
...
class ObjectMemberListDeclarationExpressionSyntaxNode extends ExpressionSyntaxNode {
  ...
  constructor(openParenToken, docComment, membersWithCommas, closeParenToken) {
    ...
    this.methods = membersWithCommas.filter(/** @return {x is ObjectMethodDeclarationStatementSyntaxNodeBase} */ x => x instanceof ObjectMethodDeclarationStatementSyntaxNodeBase)
...
class PragmaDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
  ...
  /**
   * @param {DeclarationSyntaxNode} target The target node.
   * @returns {Annotation}
   */
  createAnnotation(target) {
    return new Annotation(this.name.value.toLowerCase(), target)
  }

  /**
   * @param {DeclarationSyntaxNode} node
   * @returns {boolean}
   */
  isMatch(node) {
...
class DeprecatePragmaDeclarationStatementSyntaxNode extends PragmaDeclarationStatementSyntaxNode {
   ...
   */
  createAnnotation(target) {
    // ORACRAP: They named it `DEPRECATE` instead of `DEPRECATED`.  Really.
    return new Annotation('deprecated', target, { message: this.message })
  }
}
...
class ExceptionInitPragmaDeclarationStatementSyntaxNode extends PragmaDeclarationStatementSyntaxNode {
  ...
  createAnnotation(target) {
    ...
    return new ExceptionInitAnnotation(name, target, exception, errorCode, errorId)
  }
}
exports.ExceptionInitPragmaDeclarationStatementSyntaxNode = ExceptionInitPragmaDeclarationStatementSyntaxNode

class ExceptionInitAnnotation extends Annotation {
  /**
   * @param {IdentifierSyntaxNode} name
   * @param {DeclarationStatementSyntaxNode} target
...
class TypeExpressionSyntaxNode extends ExpressionSyntaxNode {
  ...
  constructor(...params) {
    ...
    this.typeKind = this.constructor.name.replace(/TypeExpressionSyntaxNode$/, '') || 'type'
  }
...
class RecordFieldDeclarationExpressionSyntaxNode extends DeclarationExpressionSyntaxNode {
  ...
  constructor(name, type, defaultToken, defaultValue) {
    super(name, type, defaultToken, defaultValue)
    this.name = name
    this.type = type
    this.defaultToken = this.resolveToken(defaultToken)
    this.defaultValue = defaultValue
