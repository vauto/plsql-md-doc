...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#tryReadUpToNextDocComment() {
  ...
  #tryReadNextAsObjectAttributeDeclaration() {
    ...
    return new ObjectAttributeDeclarationExpressionSyntaxNode({ name, in: inMode, out: outMode, type, defaultExpr, defaultValue })
  }

  #readNextAsObjectAttributeDeclaration() {
    const param = this.#tryReadNextAsObjectAttributeDeclaration()
    if (param) {
      return param
    }
  ...
  *#tryReadNextAsObjectAttributeDeclarationsWithCommas() {
    let param = this.#tryReadNextAsObjectAttributeDeclaration()
    if (!param) {
      return
    }
    ...
    while (param && (comma = this.tryReadNextToken(Patterns.COMMA))) {
      yield comma
      yield param = this.#readNextAsObjectAttributeDeclaration()
    }
  }

  #tryReadNextAsObjectAttributeListDeclaration() {
    const lparen = this.tryReadNextToken(Patterns.LPAREN)
    if (!lparen) {
      return null
    }

    return new ObjectAttributeListDeclarationExpressionSyntaxNode(
      lparen,
      [...this.#tryReadNextAsObjectAttributeDeclarationsWithCommas()],
      this.#readNextAsRightParen()
    )
  }
  ...
  #readNextAsTypeSpec(create, editionable, objectType) {
    ...
    if (typeType) {
      return new CreateBaseObjectTypeStatementSyntaxNode({
        ...
        authid, accessibleBy,
        is,
        attributes: this.#tryReadNextAsObjectAttributeListDeclaration(),
        modifiers: [...this.#tryReadNextAsTypeModifiers()],
        rest: this.#readAsEndOfBlock()
      })
    }
...
class CreatePackageStatementSyntaxNode extends CreatePlsqlUnitStatementSyntaxNode {
  ...
  constructor({ create, editionable, objectType, name, authid, accessibleBy, is, content }) {
    super()
    this.add({ create, editionable, objectType, name, authid, accessibleBy, is, content })
  }
}

class CreateBaseObjectTypeStatementSyntaxNode extends CreatePlsqlUnitStatementSyntaxNode {
  /** @property {SyntaxNode} */ is
  /** @property {ObjectAttributeListDeclarationExpressionSyntaxNode} */ attributes
   ...
   * @param {ObjectAttributeListDeclarationExpressionSyntaxNode} attributes
   * @param {SyntaxNode[]} modifiers
   * @param {SyntaxNode} rest
   */
  constructor({ create, editionable, objectType, name, authid, accessibleBy, is, attributes, modifiers, rest }) {
    super()
    this.add({ create, editionable, objectType, name, authid, accessibleBy, is, attributes, modifiers })
    this.push(rest)
  }
}
...
class VariableDeclarationStatementSyntaxNode extends StatementSyntaxNode {
  ...
  constructor({ name, constant, type, default: defaultNode, defaultValue, rest }) {
    super()
    this.add({ name, constant, type, default: defaultNode, defaultValue })
    this.push(rest)
  }
...
class SubtypeDeclarationStatementSyntaxNode extends TypeDeclarationStatementSyntaxNode {
  constructor({ node, name, is, specification, rest }) {
    super({ node, name, is, specification })
    this.push(rest)
  }
}
...
class ObjectAttributeDeclarationExpressionSyntaxNode extends ExpressionSyntaxNode {
...
class ObjectAttributeListDeclarationExpressionSyntaxNode extends ExpressionSyntaxNode {
  /** @type {ObjectAttributeExpressionSyntaxNode[]} */ parameters

  constructor(lparen, parametersWithCommas, rparen) {
    super()
    this.push(lparen, ...parametersWithCommas, rparen)
    this.parameters = parametersWithCommas.filter(x => x instanceof ObjectAttributeDeclarationExpressionSyntaxNode)
  }
}
exports.ObjectAttributeListDeclarationExpressionSyntaxNode = ObjectAttributeListDeclarationExpressionSyntaxNode
