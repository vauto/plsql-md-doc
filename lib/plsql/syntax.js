  ...
  *#readNextAsTypeModifiers() {
  ...
  #readAsTimestampLiteralExpression(name) {
    ...
    return new ExpressionSyntaxNode({
      ...
      precision,
      timezoneSpecifier,
      nullability: this.#tryReadNextAsNullabilityTypeRestriction()
    })
  }
...
class NullabilityTypeRestrictionExpressionSyntaxNode extends TypeRestrictionExpressionSyntaxNode {
  ...
  constructor(...tokens) {
    super(...tokens)
    console.assert(tokens.at(-1).value === 'NULL')
    console.assert(tokens.length === 1 || tokens.length === 2 && tokens[0].value === 'NOT')
