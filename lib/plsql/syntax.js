const { SyntaxNode, SyntaxNodeReader, ExpressionSyntaxNode, IdentifierSyntaxNode, StatementSyntaxNode, TokenIterator, TokenPattern, ContainerSyntaxNode } = require('../syntax')
...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#tryReadUpToNextDocComment() {
  ...
  #tryReadNextAsReturnClause() {
    const returnKeyword = this.tryReadNextToken('RETURN')
    if (!returnKeyword) {
      return null
    }

    const returnType = this.#readNextAsTypeExpression() // yeah this means we eat `varchar2(x)` but who cares.
    const pipelined = this.tryReadNextToken('PIPELINED')

    return new ExpressionSyntaxNode({ node: returnKeyword, type: returnType, pipelined })
  }
  ...
  #tryReadNextAsParameterDeclaration() {
    ...
    const inQualifier = this.tryReadNextToken('IN')
    const outQualifier = this.tryReadNextToken('OUT')
    const nocopyQualifier = this.tryReadNextToken('NOCOPY')
    ...
    return new ParameterDeclarationExpressionSyntaxNode({ name, in: inQualifier, out: outQualifier, nocopy: nocopyQualifier, type, defaultExpr, defaultValue })
  }

  *#tryReadNextAsParameterDeclarations() {
  ...
  #tryReadNextAsParameterListDeclaration() {
    ...
    return new ParameterListDeclarationExpressionSyntaxNode({
      lparen,
      parameters: [...this.#tryReadNextAsParameterDeclarations()],
      rparen: this.readNextToken(')')
    })
  }

  #readAsFunctionDeclaration(token) {
    this.verify([Patterns.FUNCTION, Patterns.PROCEDURE], token)

    const name = this.readNextAsIdentifier()
    const parameters = this.#tryReadNextAsParameterListDeclaration()
    const returnClause = this.#tryReadNextAsReturnClause()
    const rest = this.readNextTokensUntil(';')
...
class ProcedureDeclarationStatementSyntaxNode extends StatementSyntaxNode {
   ...
   */
  constructor({ node, name, parameters, rest }) {
    super({ node, name, parameters, rest })
  }
}

class FunctionDeclarationStatementSyntaxNode extends ProcedureDeclarationStatementSyntaxNode {
  /**
   *
   * @param {SyntaxNode} node
   * @param {SyntaxNode} name
   * @param {SyntaxNode?} parameters
   * @param {SyntaxNode} returnClause
   * @param {SyntaxNode?} rest
   */
  constructor({ node, name, parameters, returnClause, rest }) {
    super({ node, name, parameters, returnClause, rest })
  }
}

class ParameterDeclarationExpressionSyntaxNode extends ExpressionSyntaxNode {
  constructor({ name, in: inQualifier, out: outQualifier, nocopy, type, defaultExpr, defaultValue }) {
    super({ name, in: inQualifier, out: outQualifier, nocopy, type, defaultExpr, defaultValue })
  }
}
...
class ParameterListDeclarationExpressionSyntaxNode extends ExpressionSyntaxNode {
  ...
  constructor({ lparen, parameters, rparen }) {
    super({ lparen, parameters, rparen })
  }
}
