...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readAllExceptionHandlers() {
  ...
  #tryReadNextAsPlsqlStatement() {
    ...
    //  - foo(1,2,3)
    //  - foo.exists(1)
    const invocation = this.#readAsInvocationChain(token) // LATER: Read any value expression.
        ...
        case 'keyword':
          switch (token.value) {
            ...
            default:
              ...
              // No parens. This could be a keyword we're not handling correctly.
              console.infoOnce(token.value, `${token.textSpan}: start of value expression: { type: '${token.type}', value: '${token.value}' }`)
              return identifier;
          }
