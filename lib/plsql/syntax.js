...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#tryReadNextAsParametersWithCommas() {
  ...
  #readAsPragma(keyword) {
    ...
    return new PragmaDeclarationStatementSyntaxNode({ keyword, name, parameters, terminator: this.#readNextAsTerminator() })
  }
...
class PragmaDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
  ...
  constructor({ keyword, name, parameters, terminator }) {
    ...
    // Depending on the type of pragma, it will associate with a given piece of code.
    // This specifies a search hint.
    this.searchHint = PragmaDeclarationStatementSyntaxNode.#searchHintByName[this.name.value]
  }
