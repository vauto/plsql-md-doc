...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextTokens(count) {
  ...
  #tryReadNextAsAuthidClause() {
    let tokens = this.tryReadNextTokens('AUTHID', { type: 'identifier' })
  ...
  *#readNextAsPackageSpecContent() {
    let token
    while (token = this.tryReadNextToken([{ type: 'reserved' }, { type: 'keyword' }])) {
      console.assert(token.type === 'reserved' || token.type === 'keyword', 'wrong token type', token)
      switch (token.value) {
        case 'SUBTYPE':
        case 'TYPE':
        case 'EXCEPTION':
        ...
        case 'END':
          ...
          break
        default:
          console.warn('unexpected keyword in package spec', token)
          yield this.#readAsContainedStatement(token)
          break
      }
    }
...
class PlsqlObjectStatementSyntaxNode extends NamedStatementSyntaxNode {
  ...
  constructor(name, authid, accessibleBy, is, ...content) {
    super(name, { authid, accessibleBy, is })
