...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsInvocationParametersWithCommas() {
  ...
  #readAsIdentifierOrInvocation(token) {
    const identifier = this.#readAsIdentifier(token, '%') // can be %ROWCOUNT, etc.
  ...
  *#readAllElsifExpressions() {
    ...
    while (elsifToken = this.tryReadNextToken('ELSIF')) {
      const boolean_expression = this.#readNextAsValueExpression()
      const thenKeyword = this.readNextToken('THEN')
      const statements = [...this.#readAllPlsqlStatements()]

      yield new ExpressionSyntaxNode(elsifToken, boolean_expression, thenKeyword, statements)
    }
  }
  ...
  #tryReadNextAsSelectStatement() {
    const selectKeyword = this.tryReadNextToken('SELECT')
    return selectKeyword ? this.#readAsOpaqueSqlStatement(selectKeyword) : null
  }
  ...
  *#readAllExceptionHandlers() {
  ...
  #tryReadNextAsPlsqlStatement() {
    ...
    //  - foo(1,2,3)
    //  - foo.exists(1)
    const identifierOrInvocation = this.#readAsIdentifierOrInvocation(token)
    ...
    return new StatementSyntaxNode(identifierOrInvocation, this.#tryReadNextAsSemicolonToken())
  }
  ...
  *#readAllPlsqlStatements() {
  ...
  #tryReadNextAsMethodBody() {
    ...
    const declareSection = declarations.length ? new DeclareSectionSyntaxNode(null, ...declarations) : null
  ...
  #readAsCursorDeclarationOrDefinition(cursorKeyword) {
    ...
    // Only read the body if we find IS or AS
    const body = isOrAsKeyword ? this.#tryReadNextAsSelectStatement() : null
