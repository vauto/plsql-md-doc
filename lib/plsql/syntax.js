...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsInvocationParametersWithCommas() {
   ...
   * @param {TokenLike} functionKeyword
   * @returns {FunctionDeclarationStatementSyntaxNode}
   */
  #readAsFunctionDeclaration(functionKeyword) {
    this.verify(Patterns.FUNCTION, functionKeyword)
   ...
   * @param {TokenLike} procedureKeyword
   * @returns {ProcedureDeclarationStatementSyntaxNode}
   */
  #readAsProcedureDeclaration(procedureKeyword) {
    this.verify(Patterns.PROCEDURE, procedureKeyword)
  ...
  #readAsPragmaDeclaration(keyword) {
    ...
    return PragmaDeclarationStatementSyntaxNode.create({ keyword, name, parameterList, terminator: this.#readNextAsPlsqlTerminator() })
  }
  ...
  #readAsTypeDeclaration(typeKeyword) {
    ...
    if (record) {
      return new RecordTypeDeclarationStatementSyntaxNode(
        typeKeyword, name, is, record, this.#readNextAsParameterListDeclaration(), this.#readNextAsPlsqlTerminator()
      )
    }

    // Any other TYPE
    return new TypeDeclarationStatementSyntaxNode(typeKeyword, name, is, this.#readNextAsTypeExpression(), this.#readNextAsPlsqlTerminator())
  }
  ...
  *#readNextAsInheritanceFlags() {
  ...
  #tryReadNextAsRestOfObjectConstructorMethod(inheritance) {
    ...
    const [constructorKeyword, functionKeyword] = tokens
    return new ConstructorDeclarationStatementSyntaxNode(
      ...
      this.#readNextAsIdentifier(),
      this.#tryReadNextAsParameterListDeclaration(),
      new ConstructorReturnDeclarationExpressionSyntaxNode(...this.readNextTokens(Patterns.RETURN, 'SELF', 'AS', 'RESULT')),
      [...this.#readNextAsUnitDeclarationModifiers()]
    )
  }
  ...
  *#readNextAsTypeModifiers() {
  ...
  #readRestOfTypeSpec(create, editionable, unitType, ifNotExists) {
    ...
    let objectKeyword = this.tryReadNextToken('OBJECT')
    if (objectKeyword) {
    ...
    objectKeyword = this.tryReadNextToken('VARRAY') //, 'VARRAY', 'TABLE'])
    if (objectKeyword) {
      throw this.notImplemented('IS VARRAY')
    }

    objectKeyword = this.tryReadNextToken('TABLE')
    if (objectKeyword) {
      return new CreateNestedTableTypeStatementSyntaxNode(
        header,
        is,
        this.#tryReadNextAsDocComment(),
        this.#readAsTypeExpression(objectKeyword), // should be unrestricted but we don't care.
        [...this.#readNextAsTypeModifiers()],
        this.#tryReadNextAsSqlTerminator()
      )
    }
...
class CreatePlsqlUnitStatementSyntaxNode extends SqlStatementSyntaxNode {
  ...
  /** @property {DocumentCommentSyntaxNode?} */ docComment
   ...
   */
  constructor(header, is, ...content) {
    super(header, is, ...content)
    this.header = header
...
class CreatePackageStatementSyntaxNode extends CreatePlsqlUnitStatementSyntaxNode {
   ...
   * @param {...SyntaxNode} content
   */
  constructor(header, is, docComment, ...content) {
    super(header, is, docComment, ...content)
...
class CreateObjectTypeStatementSyntaxNode extends CreateTypeStatementSyntaxNode {
   ...
   * @param {SyntaxNode} isOrUnder
   * @param {SyntaxNode} keywordOrBaseType
   ...
   */
  constructor(header, isOrUnder, keywordOrBaseType, memberList, typeModifiers, terminator) {
    super(header, isOrUnder, keywordOrBaseType, memberList, typeModifiers, terminator)
...
class CreateBaseObjectTypeStatementSyntaxNode extends CreateObjectTypeStatementSyntaxNode {
   ...
   * @param {SyntaxNode} terminator
   */
  constructor(header, is, objectKeyword, memberList, typeModifiers, terminator) {
    super(header, is, objectKeyword, memberList, typeModifiers, terminator)
  }
}
...
class CreateInheritedObjectTypeStatementSyntaxNode extends CreateObjectTypeStatementSyntaxNode {
  /**
   * @param {CreateTypeHeaderExpressionSyntaxNode} header
   * @param {SyntaxNode} under `UNDER` keyword (as opposed to `IS`)
   * @param {IdentifierSyntaxNode} baseType
   * @param {ObjectMemberListDeclarationExpressionSyntaxNode} memberList
   * @param {SyntaxNode[]} typeModifiers
   * @param {SyntaxNode} terminator
   */
  constructor(header, under, baseType, memberList, typeModifiers, terminator) {
    super(header, under, baseType, memberList, typeModifiers, terminator)
  }
}
...
class CreateNestedTableTypeStatementSyntaxNode extends CreateTypeStatementSyntaxNode {
   ...
   * @param {SyntaxNode} terminator
   */
  constructor(header, is, docComment, baseType, typeModifiers, terminator) {
    super(header, is, docComment, baseType, typeModifiers, terminator)
    this.docComment = docComment
...
class RecordTypeDeclarationStatementSyntaxNode extends TypeDeclarationStatementSyntaxNodeBase {
   ...
   * @param {IdentifierSyntaxNode} name The identifier.
   * @param {SyntaxNode} isKeyword `IS` keyword
   ...
   * @param {...SyntaxNodeOrToken} params
   */
  constructor(typeKeyword, name, isKeyword, recordKeyword, fieldList, terminator) {
    super(typeKeyword, name, isKeyword, recordKeyword, fieldList, terminator)
...
class FunctionDeclarationStatementSyntaxNode extends MethodDeclarationStatementSyntaxNodeBase {
  /** @type {Token} `FUNCTION` */ functionKeyword
  /** @type {ReturnDeclarationExpressionSyntaxNode} */ returnClause

  /**
   * @param {TokenLike} functionKeyword `FUNCTION`
   * @param {IdentifierSyntaxNode} name
   ...
   * @param {SyntaxNode?} terminator
   */
  constructor(functionKeyword, name, parameterList, returnClause, unitModifiers, body, terminator) {
    super(functionKeyword, name, parameterList, returnClause, unitModifiers, body, terminator)
    // Must resolve the canonical tokens.
    this.functionKeyword = this.resolveToken(functionKeyword)
    this.name = name
...
class ProcedureDeclarationStatementSyntaxNode extends MethodDeclarationStatementSyntaxNodeBase {
  /** @type {Token} `PROCEDURE` */ procedureKeyword

  /**
   * @param {SyntaxNode} procedureKeyword  The initial token (`PROCEDURE`)
   * @param {IdentifierSyntaxNode} name
   * @param {DeclarationParameterListExpressionSyntaxNode?} parameterList
   * @param {SyntaxNode[]} unitModifiers
   * @param {ExpressionSyntaxNode?} body An optional body.
   * @param {SyntaxNode?} terminator
   */
  constructor(procedureKeyword, name, parameterList, unitModifiers, body, terminator) {
    super(procedureKeyword, name, parameterList, unitModifiers, body, terminator)
    this.procedureKeyword = this.resolveToken(procedureKeyword)
    this.name = name
    this.parameterList = parameterList
    this.unitModifiers = unitModifiers
    this.body = body
...
class ConstructorDeclarationStatementSyntaxNode extends ObjectMethodDeclarationStatementSyntaxNodeBase {
  /**
   * @param {InheritanceFlagSyntaxNode[]} inheritance
   * @param {TokenLike} constructorKeyword
   * @param {TokenLike} functionKeyword
   * @param {IdentifierSyntaxNode} name
   * @param {DeclarationParameterListExpressionSyntaxNode?} parameterList
   * @param {ReturnDeclarationExpressionSyntaxNode?} returnClause
   * @param {SyntaxNode[]} unitModifiers
   */
  constructor(inheritance, constructorKeyword, functionKeyword, name, parameterList, returnClause, unitModifiers) {
    super(inheritance, constructorKeyword, functionKeyword, name, parameterList, returnClause, unitModifiers)
...
class PragmaDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
   ...
   */
  static create({ keyword, name, parameterList, terminator }) {
    console.assert(keyword && name && parameterList)
    switch (name.value) {
      /** @see https://docs.oracle.com/en/database/oracle/oracle-database/23/lnpls/AUTONOMOUS_TRANSACTION-pragma.html */
      case 'AUTONOMOUS_TRANSACTION':
        return new PragmaDeclarationStatementSyntaxNode({ keyword, name, parameterList, terminator, searchHints: PragmaDeclarationStatementSyntaxNode.#searchHintsByName[name.value] })
       ...
       */
      case 'DEPRECATE':
        return new DeprecatePragmaDeclarationStatementSyntaxNode({ keyword, name, parameterList, terminator })
      case 'EXCEPTION_INIT':
        return new ExceptionInitPragmaDeclarationStatementSyntaxNode({ keyword, name, parameterList, terminator })
      /**
   ...
   */
  constructor({ keyword, name, parameterList, terminator, searchHints }) {
    super(keyword, name, parameterList, terminator)
    this.name = name
    this.#parameterList = parameterList
...
class DeprecatePragmaDeclarationStatementSyntaxNode extends PragmaDeclarationStatementSyntaxNode {
   ...
   * @param {TerminatorSyntaxNode} terminator
   */
  constructor({ keyword, name, parameterList, terminator }) {
    super({ keyword, name, parameterList, terminator })
