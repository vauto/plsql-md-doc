...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsInvocationParametersWithCommas() {
   ...
   * @param {TokenLike} keyword
   * @returns {TypeDeclarationStatementSyntaxNode}
   */
  #readAsTypeDeclaration(keyword) {
    this.verify({ value: 'TYPE' }, keyword)

    return new TypeDeclarationStatementSyntaxNode({
      keyword,
      name: this.#readNextAsIdentifier(),
      is: this.readNextToken(Patterns.IS),
      specification: this.#readNextAsTypeExpression(),
  ...
  *#readNextAsTypeModifiers() {
  ...
  #readAsTypeExpression(name) {
    ...
    switch (name.value) {
      ...
      case 'RECORD':
        // Record type (`RECORD (...)`)
        return new RecordTypeExpressionSyntaxNode(name, this.#readNextAsParameterListDeclaration())
...
class TypeDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
   ...
   * @param {...SyntaxNodeOrToken} params
   */
  constructor({ keyword, name, is, specification, terminator }, ...params) {
