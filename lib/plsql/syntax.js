  ...
  #readAsIntervalTypeExpression(name) {
    ...
    if (day) {
      return new ExpressionSyntaxNode({
        name,
        day,
        dayPrecision: this.#tryReadNextAsPrecisionTypeRestriction(),
        to: this.readNextToken('TO'),
        second: this.readNextToken('SECOND'),
        secondPrecision: this.#tryReadNextAsPrecisionTypeRestriction()
      })
    }
    ...
    if (year) {
      return new ExpressionSyntaxNode({
        name,
        year,
        yearPrecision: this.#tryReadNextAsPrecisionTypeRestriction(),
        to: this.readNextToken('TO'),
...
class DecimalTypeExpressionSyntaxNode extends TypeExpressionSyntaxNode {
  ...
  /**
   * @param {IdentifierSyntaxNode} name The type name (e.g., `VARCHAR2`).
   * @param {PrecisionAndScaleTypeRestrictionExpressionSyntaxNode?} precisionAndScale
   */
  constructor(name, precisionAndScale) {
    super(name, precisionAndScale)
  }
