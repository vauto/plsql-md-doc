...
class Patterns {
  ...
  static OF = { type: 'reserved', value: 'OF' }
...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#tryReadNextAsTypeModifiers() {
  ...
  #readNextAsTypeSpec(create, editionable, unitType) {
    ...
    if (keyword) {
      return new CreateNestedTableTypeStatementSyntaxNode({
        ...
        is,
        docComment: this.#tryReadNextAsDocComment(),
        specification: this.#readAsTypeExpression(keyword),
        modifiers: [...this.#tryReadNextAsTypeModifiers()],
        rest: this.tryReadNextToken(Patterns.END_OF_SQL_STATEMENT)
      })
    }
  ...
  #readAsCreateStatement(token) {
    ...
    switch (unitType.name) {
      ...
      default:
        console.warn(`Unit type '${unitType.name}' is not implemented`)
  ...
  #tryReadNextAsTypePrecisionExpression() {
    ...
    if (lparen) {
      return new ExpressionSyntaxNode({
        lparen,
        precision: this.readNextToken(Patterns.NUMBER_LITERAL),
        rparen: this.#readNextAsRightParen()
      })
    }
      ...
      case 'keyword':
        switch (name.value) {
          ...
          default:
            ...
            console.warn('handle this type', name.value)
  ...
  *#tryReadNextAsReturnModifiers() {
  ...
  #tryReadNextAsParameterDeclaration() {
    ...
    const defaultExpr = this.tryReadNextToken(Patterns.ANY_DEFAULT)
  ...
  * #tryReadNextAsParameterDeclarationsWithCommas() {
  ...
  #readAsVariableDeclaration(token) {
    ...
    const defaultKeyword = this.tryReadNextToken(Patterns.ANY_DEFAULT)
   ...
   */
  #readAsSqlStatement(keyword, params = {}) {
    if (params.keyword) throw new Error("Cannot specify 'keyword' property: we declare this internally")
    if (params.rest) throw new Error("Cannot specify 'rest' property: we declare this internally")
    return new StatementSyntaxNode({
      ...params,
      keyword,
      rest: this.#readNextAsEndOfSqlStatement()
    })
  }

  /**
   * Read the rest as an opaque SQL statement.  NOTE: will not work for multi-semicolon units like PL/SQL units.
   * @param {[key: string]: SyntaxNode} params Named nodes, if any
   * @returns {StatementSyntaxNode}
   */
...
class CreateBaseObjectTypeStatementSyntaxNode extends CreateTypeStatementSyntaxNode {
   ...
   */
  constructor({ create, editionable, unitType, name, force, oid, authid, accessibleBy, is, keyword, members, modifiers, rest }) {
    super(create, editionable, unitType, name, force, oid, authid, accessibleBy, is, keyword, members, modifiers, rest)
    this.unitType = unitType
    this.name = name
...
class CreateNestedTableTypeStatementSyntaxNode extends CreateTypeStatementSyntaxNode {
   ...
   * @param {SyntaxNode} params.rest
   */
  constructor({ create, editionable, unitType, name, force, oid, authid, accessibleBy, is, docComment, specification, modifiers, rest }) {
    super(create, editionable, unitType, name, force, oid, authid, accessibleBy, is, docComment, specification, modifiers, rest)
    this.unitType = unitType
    this.name = name
    this.docComment = docComment
    this.modifiers = modifiers
...
class DeclarationStatementSyntaxNode extends StatementSyntaxNode {
   ...
   * @param {NamedNodeParams} params.other
   */
...
class ExceptionDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
   ...
   * @param {NamedNodeParams} params.other
   */
  constructor({ name, keyword, terminator, ...other }) {
    super({ name, keyword, terminator, ...other })
  }
}
...
class VariableDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
   ...
   */
  constructor({ name, constant, type, defaultKeyword, defaultValue, rest, ...other }) {
    super({ name, constant, type, defaultKeyword, defaultValue, rest, ...other })
    this.constant = constant
    this.type = type

    if (defaultKeyword) {
      this.push(defaultKeyword, defaultValue)
...
class ProcedureDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
   ...
   * @param {SyntaxNode?} params.rest
   */
  constructor({ keyword, name, parameters, rest, ...other }) {
    super({ keyword, name, parameters, rest, ...other })
...
class FunctionDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
   ...
   * @param {SyntaxNode?} params.rest
   */
  constructor({ keyword, name, parameters, returnClause, rest }) {
    this.add({ keyword, name, parameters, returnClause })
    this.push(rest)
  }
}
...
class MethodDeclarationStatementSyntaxNode extends DeclarationStatementSyntaxNode {
   ...
   * @param {ReturnDeclarationExpressionSyntaxNode?} params.returnClause
   */
  constructor({ keyword, name, parameters, returnClause }) {
    super()
    this.add({ keyword, name, parameters, returnClause })
  }
}
...
class ObjectMethodDeclarationStatementSyntaxNode extends MethodDeclarationStatementSyntaxNode {

  /** @type {InheritanceFlagSyntaxNode[]} */ inheritance
  /** @type {SyntaxNode} */ memberOrStatic
  /** @param {SyntaxNode?} */ mapOrOrder
  /** @param {SyntaxNode?} */ memberOrStatic
  ...
  constructor({ inheritance, mapOrOrder, memberOrStatic, name, parameters, returnClause }) {
    super()
    this.push(inheritance, mapOrOrder, memberOrStatic, keyword, name, parameters, returnClause)
    this.name = name
...
class ReturnDeclarationExpressionSyntaxNode extends ExpressionSyntaxNode {
  ...
  constructor({ keyword, type, modifiers }) {
    super()
    this.add({ keyword, type, modifiers })
  }
}
