...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readThroughEndOfLine() {
  ...
  #readAsCreateStatement(token) {
    ...
    switch (unitType.name) {
      case 'FUNCTION':
        return this.#readRestOfStandaloneFunction(create, editionable, unitType)
      case 'PROCEDURE':
        return this.#readRestOfStandaloneProcedure(create, editionable, unitType)
      case 'PACKAGE':
        return this.#readRestOfPackageSpec(create, editionable, unitType)
      case 'TYPE':
        return this.#readRestOfTypeSpec(create, editionable, unitType)
      default:
        console.assertOnce(unitType.name, false, `${create.textSpan} Unit type '${unitType.name}' is not implemented`)
        return this.#readNextAsOpaqueSqlStatement(create, editionable, unitType)
    }
  }
  ...
  #tryReadNextAsSqlTerminator() {
    ...
    const terminatorPlusOther = this.#readNextAsOpaqueSqlStatement()
    if (terminatorPlusOther) {
      console.assert(false, `${terminatorPlusOther.start} Expected SEMICOLON or SLASH but found '${terminatorPlusOther}'`)
  ...
  /**
   * Tries reading the next item as an `AUTHID` clause.
   * ```sql
  ...
  *#readNextAsInvocationParametersWithCommas() {
  ...
  /**
   * Reads starting with the given token as an invocation of a procedure, function, cursor, method, pseudofunction, or similar.
   * @param {TokenLike} token
   * @returns {InvocationExpressionSyntaxNode}
   */
  #readAsInvocation(token) {
    return new InvocationExpressionSyntaxNode({
      name: new IdentifierSyntaxNode(token),
      parameters: this.#tryReadNextAsInvocationParameterList()
    })
  }

  /**
   * Reads the next item as as an invocation of a procedure, function, cursor, method, pseudofunction, or similar.
   * @returns {InvocationExpressionSyntaxNode}
   */
  #readNextAsInvocation() {
    const identifier = this.#readNextAsIdentifier()
    return this.#readAsInvocation(identifier)
  }
  ...
  #readAsTypeDeclaration(keyword) {
    ...
    // Any other TYPE
    return new OtherTypeDeclarationStatementSyntaxNode(keyword, name, is, this.#readNextAsTypeExpression(), this.#readNextAsPlsqlTerminator())
  }
   ...
   */
  #readRestOfStandaloneFunction(create, editionable, unitType) {
   ...
   */
  #readRestOfStandaloneProcedure(create, editionable, unitType) {
  ...
  *#readNextAsPackageSpecContent() {
        ...
        case 'keyword':
          switch (token.value) {
            ...
            case 'PRAGMA':
              ...
              continue
            case 'END':
              yield this.#readAsOpaqueSqlStatement(token)
              continue
          }
          break
      }
   ...
   */
  #readRestOfPackageSpec(create, editionable, unitType) {
  ...
  #tryReadNextAsRestOfObjectConstructorMethod(inheritance) {
    ...
    return new ConstructorDeclarationStatementSyntaxNode({
      ...
      returnClause: new ConstructorReturnDeclarationExpressionSyntaxNode({
        ...
        modifiers: [...this.#readNextAsReturnModifiers()]
      })
    })
  }
  ...
  *#readNextAsTypeModifiers() {
   ...
   * @see https://docs.oracle.com/en/database/oracle/oracle-database/23/lnpls/CREATE-TYPE-statement.html#GUID-389D603D-FBD0-452A-8414-240BBBC57034
   */
  #readRestOfTypeSpec(create, editionable, unitType) {
    // LATER: 23c: support IF NOT EXISTS
    ...
    if (keyword) {
      return new CreateBaseObjectTypeStatementSyntaxNode({
        ...
        modifiers: [...this.#readNextAsTypeModifiers()],
    ...
    if (keyword) {
      return new CreateNestedTableTypeStatementSyntaxNode({
        ...
        is,
        docComment: this.#tryReadNextAsDocComment(),
        specification: this.#readAsTypeExpression(keyword), // should be unrestricted but we don't care.
        modifiers: [...this.#readNextAsTypeModifiers()],
        terminator: this.#tryReadNextAsSqlTerminator()
      })
    }
  ...
  #readAsTypeExpression(name) {
    ...
    switch (name.value) {
      ...
      case 'TABLE':
        ...
        return new TypeExpressionSyntaxNode(
          name,
          this.readNextToken(Patterns.OF),
          this.#readNextAsTypeExpression(),
          this.#tryReadNextAsNullabilityTypeRestriction()
        )
  ...
  /**
   * Reads starting with the given token as a number literal expression.
   * @param {TokenLike} token
   * @returns {LiteralSyntaxNode}
   */
  #readAsNumberLiteralExpression(token) {
   ...
   * @returns {LiteralSyntaxNode?}
   */
  #tryReadNextAsNumberLiteralExpression() {
    const token = this.tryReadNextToken(Patterns.NUMBER_LITERAL)
    return token ? this.#readAsNumberLiteralExpression(token) : null
  }
   ...
   * @returns {LiteralSyntaxNode}
   */
  #readNextAsNumberLiteralExpression() {
    return this.#readAsNumberLiteralExpression(this.readNextToken(Patterns.NUMBER_LITERAL))
  }

  // ---------------

  /**
   * Reads starting with the given token as a number expression.
   * @param {TokenLike} name
   * @returns {ExpressionSyntaxNode | LiteralSyntaxNode}
   */
  #readAsNumberExpression(token) {
    // LATER: Oracle allows references in a lot of places now as of 19c? 21c?.
    // For now just read a number literal
    return this.#readAsNumberLiteralExpression(token)
  }
  ...
  #tryReadNextAsNumberExpression() {
    // LATER: Oracle allows references in a lot of places now as of 19c? 21c?.
    // For now just read a number literal
    return this.#tryReadNextAsNumberLiteralExpression()
  }
  ...
  #readNextAsNumberExpression() {
    // LATER: Oracle allows references in a lot of places now as of 19c? 21c?.
    // For now just read a number literal
    return this.#readNextAsNumberLiteralExpression()
  }
  ...
  #readAsIntervalLiteralExpression(keyword) {
    ...
    const dayKeyword = this.tryReadNextToken('DAY')
    if (dayKeyword) {
      return new DSIntervalLiteralExpressionSyntaxNode(
        ...
        dayKeyword,
        this.#tryReadNextAsPrecisionTypeRestriction(),
        this.readNextTokens('TO', 'SECOND'),
        this.#tryReadNextAsPrecisionTypeRestriction()
      )
    }

    const yearKeyword = this.tryReadNextToken('YEAR')
    if (yearKeyword) {
      return new YMIntervalLiteralExpressionSyntaxNode(
        ...
        yearKeyword,
        this.#tryReadNextAsPrecisionTypeRestriction(),
        this.readNextTokens('TO', 'MONTH'),
      )
    }

    throw new TokenSyntaxError(name, "INTERVAL literal found, but not DAY or YEAR")
  }
  ...
  /**
   * Reads starting with the given token as a simple value expression.
   * @param {TokenLike} token
    ...
    switch (token.type) {
      ...
      case 'keyword':
        switch (token.value) {
          ...
          // keywords that MAY be function calls
          case 'SYSDATE':
          case 'SYSTIMESTAMP':
          case 'CURRENT_DATE':
          ...
          default:
            // Treat everything as an invocation for now
            console.warnOnce(token.value, `${token.textSpan}: consider mapping ${token}`)
            return this.#readAsInvocation(token)
        }
        break
      ...
      case 'operator':
        switch (token.value) {
          case '(': {
            return this.#readAsParenthesizedValueExpression(token)
          }
  ...
  /**
   * Reads starting with the given token as as a parenthesized value expression.
   * ```text
   * (<expression>)
   * ```
   * @param {TokenLike} token
   ...
   */
  #readAsParenthesizedValueExpression(token) {
  ...
  /**
   * Tries reading the next item as a `DETERMINISTIC` clause from a return declaration modifier.
   * @returns {SyntaxNode?}
  ...
  /**
   * Tries reading the next item as a `PARALLEL_ENABLE` clause from a return declaration modifier.
   * @returns {ExpressionSyntaxNode?}
  ...
  /**
   * Tries reading the next item as a `PIPELINED` clause from a return declaration modifier.
   * @returns {ExpressionSyntaxNode?}
  ...
  /**
   * Tries reading the next item as a `RETURN` declaration modifier.
   * @returns {SyntaxNode?}
   */
  ...
  /**
   * Reads a sequence of zero or more return declaration modifiers.
   * @returns {Generator<SyntaxNode>}
   * @yields {SyntaxNode}
   */
  *#readNextAsReturnModifiers() {
    let modifier
  ...
  /**
   * Tries reading the next item as the `IN` parameter declaration mode keyword, along with additional modifiers.
   * @returns {SyntaxNode?}
   */
  #tryReadNextAsInMode() {
  ...
  /**
   * Tries reading the next item as the `OUT` parameter declaration mode keyword, along with additional modifiers.
   * @returns {SyntaxNode?}
   */
        ...
        switch (pragma.searchHint) {
          case 'parent':
            ...
            if (!pragma.tryAnnotate(this)) {
              console.warn(`${pragma.textSpan} pragma ${pragma} does not match parent ${this}`)
            }
            continue

          case 'previous':
            if (!previousDeclaration || !pragma.tryAnnotate(previousDeclaration)) {
              console.warn(`${pragma.textSpan} "previous" pragma failed to annotate item`, pragma, previousDeclaration)
            }
            break
          ...
          default:
            console.warn(`${pragma.textSpan} unsupported pragma search hint`, pragma, this)
            break
        }
      } else {
        // Not a pragma.
...
class TypeDeclarationStatementSyntaxNodeBase extends DeclarationStatementSyntaxNode {
   ...
   * @param {ExpressionSyntaxNode} specification
...
class SubtypeDeclarationStatementSyntaxNode extends TypeDeclarationStatementSyntaxNodeBase {
   ...
   * @param {SyntaxNode} terminator
   */
  constructor(subtype, name, is, specification, terminator) {
    super(subtype, name, is, specification, terminator)
 ...
 */
class OtherTypeDeclarationStatementSyntaxNode extends TypeDeclarationStatementSyntaxNodeBase {
  /** @property {ExpressionSyntaxNode} */ specification
   ...
   * @param {SyntaxNode} terminator
   */
  constructor(type, name, is, specification, terminator) {
    super(type, name, is, specification, terminator)
    this.specification = specification
  }
}
exports.OtherTypeDeclarationStatementSyntaxNode = OtherTypeDeclarationStatementSyntaxNode
...
class MethodDeclarationStatementSyntaxNodeBase extends DeclarationStatementSyntaxNode {
  ...
  /** @type {DeclarationParameterListExpressionSyntaxNode?} The parameters. */ parameters
}
...
class DeclarationParameterExpressionSyntaxNode extends ExpressionSyntaxNode {
  ...
  /** @type {SyntaxNode?} The `default` keyword or symbol (`:=`) */ defaultExpression
  /** @type {SyntaxNode?} */ defaultValue
  /** @type {Annotation[]} */ annotations = [] // LATER: implement
...
class InvocationExpressionSyntaxNode extends ExpressionSyntaxNode {
  /** @type {IdentifierSyntaxNode} */ name
  /** @type {InvocationParameterListExpressionSyntaxNode} */ parameters
...
class DSIntervalLiteralExpressionSyntaxNode extends LiteralExpressionSyntaxNode {
  /** @type {PrecisionTypeRestrictionExpressionSyntaxNode?} */ dayPrecision
  /** @type {PrecisionTypeRestrictionExpressionSyntaxNode?} */ secondPrecision

  /**
   * @param {TokenLike} intervalKeyword
   * @param {LiteralSyntaxNode} literal
   * @param {TokenLike} dayKeyword
   * @param {PrecisionTypeRestrictionExpressionSyntaxNode?} dayPrecision
   * @param {TokenLike[]} toSecondKeywords
   * @param {PrecisionTypeRestrictionExpressionSyntaxNode?} secondPrecision
   */
  constructor(intervalKeyword, literal, dayKeyword, dayPrecision, toSecondKeywords, secondPrecision) {
    super(intervalKeyword, literal, dayKeyword, dayPrecision, toSecondKeywords, secondPrecision)
...
class YMIntervalLiteralExpressionSyntaxNode extends LiteralExpressionSyntaxNode {
  /** @type {PrecisionTypeRestrictionExpressionSyntaxNode?} */ yearPrecision

  /**
   * @param {TokenLike} intervalKeyword
   * @param {LiteralSyntaxNode} literal
   * @param {TokenLike} yearKeyword
   * @param {PrecisionTypeRestrictionExpressionSyntaxNode?} yearPrecision
   * @param {TokenLike[]} toMonthKeywords
   */
  constructor(intervalKeyword, literal, yearKeyword, yearPrecision, toMonthKeywords) {
    super(intervalKeyword, literal, yearKeyword, yearPrecision, toMonthKeywords)
...
class TypeExpressionSyntaxNode extends ExpressionSyntaxNode {
  /** @type {IdentifierSyntaxNode} The name of the type expression */ name
