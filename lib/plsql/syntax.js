...
class Patterns {
  ...
  static PERIOD = this.operator('.')
  static COMMA = this.operator(',')
...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readThroughEndOfLine() {
   ...
   */
  #readAsUnitType(tokens) {
    return new UnitTypeSyntaxNode(tokens)
  }
  ...
  #readNextAsUnitType() {
    ...
    // General case
    return this.#readAsUnitType(this.readNextToken(Patterns.ANY_KEYWORD))
  }
  ...
  #tryReadNextAsAccessorUnitKind() {
    return this.tryReadNextAs(this.#readAsUnitType, Patterns.PLSQL_UNIT_KIND)
  }
  ...
  *#readNextAsInvocationParametersWithCommas() {
  ...
  #readAsInvocationChain(token) {
    const identifier = this.#readAsIdentifier(token, '@', '%') // can be %ROWCOUNT, @myDB, etc.
    ...
    // May be a single invocation or multiple.
    const invocation = new InvocationExpressionSyntaxNode(identifier, parameterList)

    const dotToken = this.tryReadNextToken(Patterns.PERIOD)
    if (dotToken) {
      return new BinaryExpressionSyntaxNode(invocation, dotToken, this.#tryReadNextAsInvocationChain())
    }
  ...
  *#readAllDeclarations() {
  ...
  /**
   * @param {TokenLike} loopToken
   * @returns {StatementSyntaxNode}
   */
  #readAsBasicLoopStatement(loopToken) {
    // No frills or checks yet.
    return new StatementSyntaxNode(loopToken,
      ...this.#readAllPlsqlStatements(),
      this.readNextTokens('END', 'LOOP', ';')
    )
  }

  /**
   * @param {TokenLike} forToken
   * @returns {StatementSyntaxNode}
   ...
   */
  #readAsForLoopStatement(forToken) {
    // No frills or checks yet.
    const indexInRangeLoop = this.readNextTokensUntil('LOOP')
    return new StatementSyntaxNode(forToken,
      ...indexInRangeLoop,
      ...this.#readAllPlsqlStatements(),
      this.readNextTokens('END', 'LOOP', ';')
    )
  ...
  *#readAllElsifExpressions() {
  ...
  #tryReadNextWhenThenClause() {
    ...
    return new ExpressionSyntaxNode(
      whenToken,
      // Not really in the mood to deal with selector_value | dangling_predicate.
      // Read through the 'THEN' token.
      ...this.readNextTokensUntil('THEN'),
      // These can be any PL/SQL statements. Of course.
      ...this.#readAllPlsqlStatements()
    )
  }
  ...
  #readAsCaseStatement(caseToken) {
    ...
    console.assert(selector?.toString('V') !== 'WHEN', 'oops WHEN')

    // LATER: Split WHEN..THEN clause into two if we have to parse the expression.
    const clauses = [...this.readAllAs(this.#tryReadNextWhenThenClause)]

    const elseKeyword = this.tryReadNextToken('ELSE')
    const elseClause = elseKeyword
      ? new ExpressionSyntaxNode(elseKeyword, ...this.#readAllPlsqlStatements())
      : null
    ...
    return new StatementSyntaxNode(
      ...
      ...clauses,
      elseClause,
      new ExpressionSyntaxNode(
        this.readNextTokens('END', 'CASE'),
  ...
  #readAsIfStatement(ifToken) {
    return new StatementSyntaxNode(
      ...
      this.readNextTokens('END', 'IF', ';') // END IF ;
    )
  }
  ...
  #readAsSelectStatement(selectOrWithKeyword) {
    return this.#readAsOpaqueSqlStatement(selectOrWithKeyword)
  }
  ...
  *#readAllExceptionHandlers() {
  ...
  #tryReadNextAsPlsqlStatement() {
    ...
    //  - foo(1,2,3)
    //  - foo.exists(1)
    const invocation = this.#readAsInvocationChain(token)
    ...
    return new StatementSyntaxNode(invocation, this.#tryReadNextAsSemicolonToken())
  }
  ...
  *#readAllPlsqlStatements() {
    let statement
    while (statement = this.#tryReadNextAsPlsqlStatement()) {
      yield statement
    }
  }
  ...
  #readAsMethodDeclaration(methodKeyword) {
    ...
    const unitType = this.#readAsUnitType(methodKeyword),
  ...
  *#readNextAsInheritanceFlags() {
  ...
  #tryReadNextAsRestOfConstructorDeclaration(inheritance) {
    ...
    return new ConstructorDeclarationExpressionSyntaxNode(
      inheritance,
      constructorKeyword,
      this.#readAsUnitType(methodKeyword),
      this.#readNextAsIdentifier(),
  ...
  *#readNextAsTypeModifiers() {
  ...
  #readAsTypeExpression(name) {
    ...
    if (this.nextIs(Patterns.PERIOD)) {
      ...
      switch (token.type) {
        ...
        case 'keyword':
          switch (token.value) {
            ...
            case 'TO_DATE':
            case 'TO_NUMBER':
              return this.#readAsInvocationChain(token)
            default:
  ...
  *#readNextAsRecordFieldDeclarationsWithCommas() {
  ...
  readInternal() {
    ...
    while (token = this.tryReadNextToken()) {
      ...
      console.assert(false, `${token.textSpan} unrecognized token '${token.value}'`, token)
      return this.#readAsOpaqueSqlStatement(token)
    }
  }
}
...
class StatementSyntaxNode extends StatementSyntaxNodeBase {
  ...
  constructor(...nodesOrTokens) {
    ...
    switch (this.lastNontrivialToken.value) {
      ...
      default:
        // ???
        console.info('semicolon not found for statement', this)
        break
    }
  }
}
