...
const { mustBeNonEmptyArray, mustBeInstanceOf, InvalidNumberOfArgumentsError, mustBeArray, mustBeObject, mustBeNonEmptyString } = require('../guards')
const {
  Annotation,
  ...
  *#readThroughEndOfLine() {
  ...
  #readNextAsUnitType() {
    ...
    if (next = this.tryReadNextTokens('MATERIALIZED', 'VIEW')) {
      ...
      return new UnitTypeSyntaxNode(next, next[1])
    }
...
class UnitTypeSyntaxNode extends SyntaxNode {
   ...
   * @param {UnitType?} name How Oracle refers to this.  Oracle has compound keywords like `PUBLIC SYNONYM` and `PACKAGE BODY`, but in those the primary ones are `SYNONYM` and `PACKAGE BODY` respectively.
   */
