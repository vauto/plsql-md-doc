...
class PlsqlNodeReader extends SyntaxNodeReader {
  ...
  *#readNextAsTypeModifiers() {
  ...
  /**
   * Reads starting with the given token as a TABLE type expression.
   * ```text
   * TABLE OF <type> [INDEX BY <type>]?
   * ```
   * @param {TokenLike} name
   * @returns {TypeExpressionSyntaxNode}
   * @see https://docs.oracle.com/en/database/oracle/oracle-database/23/lnpls/collection-variable.html#GUID-89A1863C-65A1-40CF-9392-86E9FDC21BE9
   */
  #readAsTableOfTypeExpression(table) {
    ...
    if (indexBy) {
      ...
      return new NestedTableTypeExpressionSyntaxNode(tableOf, itemType)
    }
  }
      ...
      if (pragma) {
        switch (pragma.searchHint) {
          ...
          case 'previous':
            ...
            console.warn(`${pragma.textSpan} "previous" pragma failed to annotate item`, pragma)
            break
...
class TypeDeclarationStatementSyntaxNode extends TypeDeclarationStatementSyntaxNodeBase {
   ...
   * @param {IdentifierSyntaxNode} name The identifier.
   * @param {SyntaxNode} is `IS` keyword
   * @param {TypeExpressionSyntaxNode} specification
   * @param {SyntaxNode} terminator
   */
