...
class UnitData {
  ...
  diff(other) {
    ...
    for (const key of keys) {
      ...
      switch (typeof thisValue) {
        case 'object':
          if (thisValue instanceof Array && otherValue instanceof Array) {
            ...
            const resultValue = { deleted: thisValue.filter(item => otherValue.indexOf(item) === -1) }
            if (resultValue.deleted.length > 0) {
      ...
      result[key] = [thisValue, otherValue]
    }
...
pmd.processFile = function (file) {
  ...
  content.comments = [...dox.parseComments(content.data, { filename: file.path })];
...
pmd.readFolder = function (objs, config, folder) {
  ...
  for (const sourceFile of files) {
    ...
    const entities = pmd.processFile(file, debug);
...
pmd.mergeObjs = function (objs, { excludeVisiblity = ['private', 'internal'] } = {}) {
  objs.forEach(function (obj, i) {
    ...
    if (data !== filteredData) {
      ...
      console.log(data.diff(filteredData))
    }
  }); //objs.forEach
