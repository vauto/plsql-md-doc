...
const { console } = require('./debug.js')
  ...
  for (const comment of comments) {
    ...
    for (const tag of comment.tags) {
      switch (tag.kind) {
        ...
        case 'param':
          {
            const param = entity.params.find(p => p.name.equals(tag.name))
            if (param) {
...
pmd.readFolder = function (objs, config, folder) {
  ...
  for (const sourceFile of files) {
    ...
    const entities = pmd.processFile(file);
      ...
      data.methods.forEach(function (myData) {
        relatedData.methods.forEach(function (myRelatedElement, j) {
          if (myData.name === myRelatedElement.name) {
            console.log('Deleting common method:', myData.name);
