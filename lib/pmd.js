...
pmd.processFile = function (file) {
  ...
  for (var comment of content.json) {
    ...
    if (comment.ignore) {
      debug.log('Ignoring:', comment.ctx);
    ...
    if (comment.ctx) {
      entity.name = comment.ctx.name
      entity.type = comment.ctx.type
      entity.specification = comment.ctx.specification
      entity.defaultValue = comment.ctx.defaultValue
    }
    else {
    ...
    entity.description = comment.description ? comment.description : '';
    entity.code = comment.code ? comment.code.join('') : '';


    if (entity.code && entity.type === pmd.DOCTYPES.EXCEPTIONS) {
      entity.exceptions = comment.ctx.exceptions;
    ...
    if (entity.code && (entity.type === pmd.DOCTYPES.FUNCTION || entity.type === pmd.DOCTYPES.PROCEDURE || entity.type == pmd.DOCTYPES.CURSOR || entity.type == pmd.DOCTYPES.RECORD)) {
      ...
      entity.name = makeIdUnique(comment.ctx.name);
      entity.displayName = comment.ctx.name;
      ...
      entity.header = comment.ctx.header;
