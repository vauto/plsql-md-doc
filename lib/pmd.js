    ...
    case 'param':
      {
        ...
        if (param) {
          ...
          if (tag.name) {
            console.info(`${tag.textSpan} @param not found for ${entity.id}: ${tag.id}`, tag);
          } else {
 ...
 *
 * @param {string} file The fully qualified path for the source file
...
pmd.readEntitiesFromFile = function* (file) {
  ...
  const comments = [...dox.parseComments(data, { filename: file })]
...
pmd.readFolder = function* (config, folder) {
  // LATER: is there something obvious I'm missing? Neither Dirent nor ParsedPath have a "fullName" that is the joined name of the "path"/"name" or "dir"/"base" respectively.
  const entries = fs.readdirSync(path.resolve(folder.source.path), { withFileTypes: true })
    .map(e => Object.assign(e, { fullName: path.join(e.path, e.name) }))
  ...
  // LATER: see above note re: ParsedPath
  const sourceFile = Object.assign(path.parse(entry.fullName), { fullName: entry.fullName })
    if (folder.source.fileFilterRegexp instanceof RegExp && !folder.source.fileFilterRegexp.test(sourceFile.fullName)) {
      continue
    }
    ...
    for (const unit of pmd.readUnitsFromFile(sourceFile.fullName)) {
      ...
      if (!unit.docFileName) {
        ...
        if (unit.name) {
          ...
          unit.docFileName = `${sourceFile.name}-${unit.textSpan.start.line}${folder.output.ext}`
        }
      }
    ...
    if (debugUnits) {
      console.logFile(sourceFile.base + '.json', JSON.stringify(debugUnits, (key, value) => {
        switch (key) {
          case 'parent':
            return value?.id
          default:
...
pmd.mergeObjs = function (objs, options) {
  objs.forEach(function (obj, i) {
    ...
    for (var j = i + 1; j < objs.length; j++) {
      if (objs[j].unit.name.equals(unit.name)) {
