...
pmd.processFile = function* (file) {
  ...
  for (const comment of comments) {
    ...
    for (const tag of comment.tags) {
      // FIXME we need to do more resolution here
      const contentText = tag.content?.join('').trim()
      ...
      switch (tag.kind) {
        ...
        case 'created':
          ...
          break;
        case 'example':
          entity.examples.push(contentText)
          break;
        // Future: Devnotes
        // case 'devnotes':
        ...
        case 'returns':
          if (entity.return) {
            entity.return.description = contentText
          } else {
