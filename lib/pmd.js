...
pmd.DOCTYPES = {
  ...
  //Note: For constants or subtypes to be triggered @constants or @types needs to be include
  CONSTANTS: "constants",
  SUBTYPES: "subtypes",
  VARIABLES: "variables",
...
pmd.processFile = function (file) {
  ...
  content.json = [...dox.parseComments(content.data, { path: file.path })];
  for (const c of content.json) {
    console.log('comment', { code: c.code.join(' '), tokens: c.tokens })
  }

  content.entities = []; //Holds list of entities for the object

  for (var jsonData of content.json) {
    ...
    entity.code = jsonData.code ? jsonData.code.join('') : '';

    if (entity.code && entity.type === pmd.DOCTYPES.SUBTYPES) {

      entity.types = jsonData.ctx.types;
      // Loop over tags to see if there's one for this typeName
      for (var type of entity.types) {
        type.isPrivate = entity.isPrivate;

        tagTypes.forEach(function (tagType) {
          if (type.name === tagType.name) {
            type.description = tagType.description
          }
        });//tagTypes.forEach

        // Fallback description from the entity
        type.description ??= entity.description?.full ?? ''
      }// entity.types

    }//entity.code && entity.type === pmd.DOCTYPES.SUBTYPES
    ...
    if (entity.code && entity.type === pmd.DOCTYPES.CONSTANTS) {
      ...
      for (var constant of entity.constants) {
        constant.isPrivate = entity.isPrivate;

        tagConstants.forEach(function (constantType) {
          if (constant.name === constantType.name) {
            constant.description = constantType.description
          }
        });//tagConstants.forEach

        // Fallback description from the entity
        constant.description ??= entity.description?.full ?? ''
      }// entity.constants
    ...
    if (entity.code && entity.type === pmd.DOCTYPES.VARIABLES) {
      ...
      for (var variable of entity.variables) {
        variable.isPrivate = entity.isPrivate;
        console.log('var', variable, entity)

        variables.forEach(function (varType) {
          if (variable.name === varType.name) {
            variable.description = varType.description
          }
        });//tagVariables.forEach

        // Fallback description from the entity
        variable.description ??= entity.description?.full ?? ''
      } // entity.variables
    ...
    if (entity.code && (entity.type === pmd.DOCTYPES.FUNCTION || entity.type === pmd.DOCTYPES.PROCEDURE || entity.type == pmd.DOCTYPES.CURSOR || entity.type == pmd.DOCTYPES.RECORD)) {
