...
const { ItemName } = require('./name.js');
    ...
    for (const tag of comment.tags) {
      switch (tag.kind) {
        ...
        case 'param':
          {
            ...
            if (param) {
              ...
              if (tag.name) {
                console.log(`${tag.textSpan} @param not found: ${tag.name}`, tag);
              } else {
                console.log(`${tag.textSpan} @param found, but it does not have a name`, tag);
              }
            }
          }
          break;
        ...
        case 'virtual':
          ...
          break
        default:
          console.infoOnce(tag.kind, `${entity.textSpan} not handling tag`, { kind: tag.kind }, 'for', entity)
          break
      }//switch
    }
