...
const { Description, CodeContext, LinkTag, Tag, ParamTag, IncludeTag, VisibilityTag, ThrowsExceptionTag, Annotation } = require('./comment.js');
const Handlebars = require('handlebars');
const { PlsqlUnitName } = require('./plsql/name.js');
const { mustBeObject, mustBeInstanceOf } = require('./guards');
const dox = new DocumentGenerator()
const { Diffable, EntityLookup, Entity, EntityGroup, UnitEntity } = require("./entity");
 ...
 * @property {number} index The index of the unit in the original file
...
pmd.applyTag = function (entity, tag) {
  ...
  switch (tag.kind) {
    ...
    case 'virtual': // for pseudo-virtual
      entity.annotations.push(new Annotation(tag, entity.context, contentText))
      break
  ...
  for (const sourceFile of files) {
    ...
    for (const unit of pmd.readUnitsFromFile(file)) {
      ...
      if (!unit.docFileName) {
        ...
        if (unit.name) {
          unit.docFileName = `${unit.name.toString('FILE')}${docExtName}`
        } else if (fileIndex > 0) {
          unit.docFileName = `${file.base}-${fileIndex}${docExtName}`
        } else {
          // 0th file does not say `-0`.
          unit.docFileName = `${file.base}${docExtName}`
        }
      }
...
pmd.mergeObjs = function (objs, options) {
  objs.forEach(function (obj, i) {
    ...
    if (defaultDescription) {
      unit.description ||= defaultDescription
      ...
      unit.exceptions.forEach(t => t.description ||= defaultDescription)
      unit.constants.forEach(t => t.description ||= defaultDescription)
      unit.variables.forEach(t => t.description ||= defaultDescription)
      unit.methods.forEach(t => t.description ||= defaultDescription)
    }
...
pmd.saveToFile = function (config, objs) {
  ...
  for (const obj of objs) {
    obj.unit.files = pmd.globalFiles;

    var markdown = obj.template(obj.unit);

    if (console.debug) {
      console.logFile(obj.unit.docFileName, markdown);
    }

    const outFullName = path.resolve(obj.folder.output.path, obj.unit.docFileName)
