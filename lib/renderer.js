...
const { CodeTag, IncludeTag, LinkTag, Tag } = require("./comment")
  ...
  *#doRender(...items) {
    for (const item of items) {
      ...
      switch (true) {
        ...
        case item instanceof CodeTag:
          {
            // Wrap in code.
            const text = [...this.#doRender(...item.content)].join('').trim()
            yield this.#wrapCode(text)
            break
          }
