...
class ContentRenderer {
   ...
   */
  #wrapCode(text) {
    console.assert(text.indexOf('`') === -1, 'need to handle backtick situation')
  ...
  #doRenderLink(text, url, style) {
    ...
    const formattedText = this.#formatLinkText(innerText, style)

    // Only linkify if there is a URL
    return url
      ? `${leading}[${formattedText}](${url})${trailing}`
      : `${leading}${formattedText}${trailing}`
  }
    ...
    for (const item of items) {
      ...
      switch (true) {
        ...
        case item instanceof LinkTag:
          const style = this.#linkTagStyles[item.kind] ?? 'plain'
