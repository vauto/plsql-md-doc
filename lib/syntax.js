...
const { mustBeObject, mustBeInstanceOf, mustBeNonEmptyString, ArgumentValueError } = require("./guards")
...
class SyntaxNode {
  ...
  *getDocumentComments() {
  ...
  /**
   * @template {SyntaxNodeOrToken} TResultSyntaxNodeOrToken
   * @param {(value: SyntaxNodeOrToken) => value is TResultSyntaxNodeOrToken} predicate
   * @returns {TResultSyntaxNodeOrToken?}
   */
 ...
 *
 * @typedef _NamedSyntaxNode
 * @property {IdentifierSyntaxNode} name The name associated with this node.
 * @typedef {SyntaxNode & _NamedSyntaxNode} NamedSyntaxNode
 */


/**
 * @param {any} value
 * @returns {value is NamedSyntaxNode}
 */
const isNamedSyntaxNode = (value) => value instanceof SyntaxNode
  && 'name' in value
  && value.name instanceof IdentifierSyntaxNode
exports.isNamedSyntaxNode = isNamedSyntaxNode

/**
 * @param {any} value
 * @param {string} paramName
 * @returns {asserts value is NamedSyntaxNode}
 */
const mustBeNamedSyntaxNode = (value, paramName = 'value') => {
  if (!isNamedSyntaxNode(value)) {
    throw new ArgumentValueError(paramName, value, 'Value must be a NamedSyntaxNode.')
  }
}
exports.mustBeNamedSyntaxNode = mustBeNamedSyntaxNode
...
class SyntaxNodeReader {
  ...
  /** @protected @deprecated Need to stop referencing the iterator directly in so many spots */
  get iterator() {
  ...
  /**
   * Returns whether the next token matches the given pattern.
   * @param {TokenPattern} pattern
   * @returns {boolean}
   */
  nextIs(pattern) {
    return this.#iterator.nextNonTrivialIs(pattern)
  }
   ...
   * @returns {TokenLike?}
   * @overload
   * @param {TokenPattern} pattern The pattern to match.
   * @returns {TokenLike?}
   */
  tryReadNextToken(/** @type {TokenPattern?} */ pattern = undefined) {
    switch (arguments.length) {
      case 0:
        // Any token, pattern doesn't matter
        return this.#iterator.nextNonTrivial().value
      default:
        if (this.nextIs(pattern)) {
  ...
  /**
   * @template {SyntaxNode} TSyntaxNode
   * Reads the next item.
   * If the next token matches the pattern, the reader function will read it and more into a syntax node;
   * otherwise, an exception is thrown.
   * @param {(token: TokenLike) => TSyntaxNode} readerFunction The function to read the node.
   * @param {TokenPattern} pattern The pattern.
   * @returns {TSyntaxNode} The read node.
   * @throws {TokenSyntaxError} if token is not found.
   */
  readNextAs(readerFunction, pattern) {
    return readerFunction.call(this, this.readNextToken(pattern))
  }

  /**
   * Unreads the previous token.
   * @remarks
   ...
   * This is an ugly hack to work around that for now..
   */
  unreadToken() {
    const item = this.#iterator.previous()
    if (item.done) {
      throw this.syntaxError('At BOF')
    }
  }
   ...
   */
  syntaxError() {
    const defaultToken = this.#iterator.lastValue
    ...
    switch (arguments.length) {
      ...
      case 1:
        // (description)
        [token, description] = [defaultToken, arguments[0]]
        break
      case 2:
        ...
        if (typeof token === 'string') {
          ...
          [token, description, innerError] = [defaultToken, token, description]
        }
        break
      default:
    ...
    return new TokenSyntaxError(token ?? defaultToken, description, innerError)
  }
  ...
  /**
   * @template {SyntaxNode} TSyntaxNode
   * Tries reading the next item.
   * If the next tokens match the patterns, the reader function will read it and more into a syntax node.
   * @param {...TokenPattern} patterns The patterns.
   * @param {(...token: TokenLike[]) => TSyntaxNode} readerFunction The function to read the node.
   * @returns {TSyntaxNode?} The read node, or null
   */
  tryReadNextAs(readerFunction, ...patterns) {
    const tokens = this.tryReadNextTokens(patterns)
    return tokens ? readerFunction.call(this, ...tokens) : null
  }
  ...
  *readNextTokensUntil(pattern) {
  ...
  /**
   * Reads more tokens until `pattern`, exclusive, throwing an exception on EOF.
   * @param {TokenPattern} pattern
   * @returns {Generator<TokenLike>}
   */
  *readUpTo(pattern) {
    while (!this.nextIs(pattern)) {
      yield this.readNextToken()
    }
    console.assert(this.#iterator.done || this.nextIs(pattern), 'whoops pattern')
  }

  /**
   * @template {SyntaxNode} TSyntaxNode
   * Tries reading the next items as a repeated list of the same thing.
   * As long as a non-null value is returned, the reader function will read it and more into a syntax node.
   * @param {() => TSyntaxNode} readerFunction The function to read the node.
   * @returns {Generator<TSyntaxNode>} The read nodes
   * @yields {TSyntaxNode}
   */
  *readAllAs(readerFunction) {
    let item
    while (item = readerFunction.call(this)) {
      yield item
    }
  }
  /**
