...
const { mustBeObject, mustBeArray } = require("./guards")
const { Position, TextSpan } = require("./position")
const { flattenTokens, isTokenLike, isToken, Token, TokenIterator, TokenLike, TokenSyntaxError, stringifyTokenArray } = require("./token")
/**
...
class Annotation {
  ...
  /** @type {ExpressionSyntaxNode[]} */ parameters
  /** @type {SyntaxNode} The target node to which this applies. */ target

  /**
   * @param {object} param0
   * @param {IdentifierSyntaxNode} param0.name
   * @param {ExpressionSyntaxNode[]} param0.parameters
   * @param {SyntaxNode} param0.target
   * @param {{[key: string]: SyntaxNode}} param0.params
   */
  constructor({ name, parameters, target, ...params }) {
    this.name = mustBeObject(name, 'name')
    this.parameters = mustBeArray(parameters, 'parameters')
