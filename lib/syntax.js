...
class SyntaxNode {
  ...
  push(...tokens) {
    ...
    console.log('pushed', TextSpan.from(this.allTokens[start]?.start, this.end).toString(), this.constructor.name, JSON.stringify(this.toString('V')))
  }
  ...
  /** @type {string} The logical value of the token */
  get value() {
    console.assert(false, 'new value who dis')
...
class ContainerSyntaxNode extends SyntaxNode {
    ...
    for (const nodeOrToken of nodesOrTokens.flat()) {
      try {
        if (nodeOrToken === undefined || nodeOrToken === null) {
          // Just eat the null, at this point I don't care.
          continue
        }
        ...
        throw new NodeSyntaxError(nodeOrToken ?? this.lastToken, e.message, e)
      } finally {
        this.#assertSumOfParts()
      }
    }
  ...
  add(params = {}) {
    if (params instanceof SyntaxNode) throw new ArgumentError('params', 'expected dictionary not node')
    if (isTokenLike(params)) throw new ArgumentError('params', 'expected dictionary not token')
