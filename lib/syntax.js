...
class SyntaxNode {
  ...
  get structuredChildren() {
    return this.allChildren.filter(snot => snot.isTrivia !== true)
  }

  /** @returns {SyntaxNodeOrToken[]} Nontrivial children. */
  get children() {
    return this.allChildren.filter(snot => !snot.isTrivia)
  }
  ...
  /**
   * Mix in separators in the children.
   * @param {Iterable<SyntaxNodeOrToken>} children
   * @yields {(SyntaxNodeOrToken|string)}
   */
  *#separateChildren(children) {
  ...
  toString(format = null) {
    ...
    for (const item of this.#separateChildren(this.children)) {
  ...
  toStructuredString(format = null) {
    ...
    for (const item of this.#separateChildren(this.structuredChildren)) {
      result += item instanceof SyntaxNode ? item.toStructuredString(format) : item.toString(format)
    }
