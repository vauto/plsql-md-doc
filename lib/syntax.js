...
const { mustBeObject, mustBeInstanceOf, mustBeNonEmptyString } = require("./guards")
...
class Annotation {
  /** @type {string} The annotation name. */ name
   ...
   * @param {object} params
   * @param {AnyExpressionSyntaxNode} [params.message]
   */
  constructor(name, target, params = {}) {
    mustBeNonEmptyString(name, 'name')
    mustBeInstanceOf(target, SyntaxNode, 'target')
    mustBeInstanceOf(target.name, IdentifierSyntaxNode, 'target.name')
    this.name = name
    this.target = target
