...
class SyntaxNodeReader {
  ...
  verify(pattern, token) {
    if (!this.matches(pattern, token)) {
      ...
      throw new Error(`Expected: ${JSON.stringify(pattern)} but got ${JSON.stringify({ type: token.type, value: token.value })}`)
    }
  }
  ...
  *readNextTokensUntil(pattern) {
  ...
  read() {
    try {
      ...
      throw new Error(`${this.iterator.value?.start} ${e.message}`, e)
    }
  }
