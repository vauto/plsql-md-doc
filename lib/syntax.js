...
const { mustBeObject, mustBeArray, mustBeNonEmptyString } = require("./guards")
const { ItemName } = require('./name')
const { Position, TextSpan } = require("./position")
const { flattenTokens, isTokenLike, isToken, Token, TokenFormat, TokenIterator, TokenLike, TokenSyntaxError, stringifyTokenArray } = require("./token")
/**
...
class ContainerSyntaxNode extends SyntaxNode {
  ...
  *#push(...nodesOrTokens) {
    ...
    for (const nodeOrToken of nodesOrTokens.flat()) {
      ...
      console.assert(false, `${Object.values(nodeOrToken).at(0)?.textSpan ?? this.textSpan} named params`, this.constructor.name, Object.keys(nodeOrToken).sort())
