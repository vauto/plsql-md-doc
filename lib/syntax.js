...
class TokenIterator {
   ...
   */
  skip(count = 1) {
    if (typeof count !== 'number') throw new InvalidArgumentTypeOfError('count', count)
    if (count <= 0 || isNaN(count)) throw new ArgumentError('count', 'Value must be positive')
    if (this.done) {
      // nothing to skip.
      return
    }
...
//      console.warn('named param', this.constructor.name, key, value)
      try {
  ...
  push(...nodesOrTokens) {
    ...
    for (const nodeOrToken of nodesOrTokens.flat()) {
      ...
      if (isTokenLike(nodeOrToken)) {
        this.#addChildNode(SyntaxNode.from(nodeOrToken), nodeOrToken.start.toString())
        continue
      }

      if (Symbol.iterator in nodeOrToken) {
        this.push(...nodeOrToken)
        continue
      }

      console.warn('named params', this.constructor.name)
      this.#addNamedParams(nodeOrToken)
    }
  ...
  #assertContents() {
    const discontinuity = this.children
      ...
      .filter(entry => entry.previous && !entry.previous.end.equals(entry.current.start))
    if (discontinuity.length > 0) {
      const details = discontinuity.map(({ index, current, previous }) => `- [${index}]: end ${previous.end} does not match ${current.start}`)
      console.assert(discontinuity.length === 0, `Children discontinuity!\n${details.join('\n')}`)
    }
