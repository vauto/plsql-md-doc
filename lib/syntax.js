...
const { mustBeObject, mustBeNonEmptyString } = require("./guards")
const { Position, TextSpan } = require("./position")
const { Token, TokenIterator, TokenSyntaxError, flattenTokens, isTokenLike, stringifyTokenArray } = require("./token")
/**
...
class SyntaxNode {
   ...
   * a sequence of {@link SyntaxNode} objects is returned as a {@link ContainerSyntaxNode}.
   */
  static from(value) {
    ...
    if (Symbol.iterator in value) {
      ...
      if (array.every(isTokenLike)) {
        ...
        return new ContainerSyntaxNode(...array)
      }
    }
  ...
  push(...tokens) {
    ...
    this.allTokens.push(...flattenTokens(tokens))
    this.assertTokenContinuity()
  }
...
class ContainerSyntaxNode extends SyntaxNode {
  ...
  *#push(...nodesOrTokens) {
   ...
   * @return {string}
   * <p>
   * By default, ContainerSyntaxNode converts to string based on whether token types require whitespace between them,
   * including between children (end of child N / start of child N+1).
   */
  toString(format = null) {
...
class SyntaxNodeReader {
  ...
  next() {
    ...
    return  /** @type {IteratorResult} */ { done: value === undefined, value }
  }
  ...
  #patternToString(pattern) {
    if (Array.isArray(pattern)) {
      return pattern.map(this.#patternToString).join('\n -- OR -- ')
    }
  ...
  verify(pattern, token) {
    if (!this.matches(pattern, token)) {
      ...
      throw new TokenSyntaxError(token, `\nExpected: ${this.#patternToString(pattern)}\n but got: ${this.#patternToString({ type: token.type, value: token.value })}`)
    }
  }
...
class Annotation {
  /** @type {string} */ name
  /** @type {SyntaxNode} The target node to which this applies. */ target
  /** @type {string?} An optional message. */ message
   ...
   */
  constructor(name, target, message, params) {
    this.name = mustBeNonEmptyString(name, 'name').toLowerCase()
    this.target = mustBeObject(target, 'target')
