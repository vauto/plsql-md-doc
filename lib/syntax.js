...
class SyntaxNode {
  ...
  get start() {
    const token = this.allTokens[0]
    return token ? new Position(token.line, token.col) : null
  }
  get end() {
    const token = this.allTokens.at(-1)
    if (!token) {
      return null
    }
...
class SyntaxNodeReader {
  /**
   * @param {TokenIterator | Iterator<Token>} iterator
   */
  constructor(iterator) {
    this.iterator = iterator instanceof TokenIterator ? iterator : new TokenIterator(iterator)
  }
   ...
   * @param {Token?} token  The token (defaults to the current {@link SyntaxNodeReader#token})
   */
  verify(pattern, token = this.token) {
