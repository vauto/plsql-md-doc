...
class NodeSyntaxError extends Error {
  ...
  /**
   *
   * @param {TokenLike?} token The offending token
...
class SyntaxNodeReader {
  ...
  endOfStreamError() {
    return new NodeSyntaxError(null, "Expected token but found EOF")
  }
  ...
  *readNextTokensUntil(pattern) {
  ...
  read() {
    try {
      ...
      throw new NodeSyntaxError(this.iterator.value, e.message, e)
    }
  }
