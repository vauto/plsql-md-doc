...
const needsWhitespace = (nodeOrToken) => {
  switch (nodeOrToken.type) {
    case 'identifier':
    case 'reserved':
    case 'keyword':
    case 'number':
...
function stringifySyntaxNodeOrTokenArray(nodesOrTokens, format) {
  const formatter = tokenFormatters[format?.toUpperCase() ?? '']

  let prev = {}
  let result = ''
  for (const nodeOrToken of nodesOrTokens) {
    const needsSpace = needsWhitespace(nodeOrToken)
    if (needsSpace && prev.needsSpace) {
      result += ' '
    }
    result += formatter(nodeOrToken)

    prev = { token: nodeOrToken, needsSpace }
  }
...
class SyntaxNode {
  ...
  toString(format = null) {
    return stringifySyntaxNodeOrTokenArray(this.tokens, format)
  }
  ...
  toFullString(format = null) {
    return stringifySyntaxNodeOrTokenArray(this.allTokens, format)
  }
...
class ContainerSyntaxNode extends SyntaxNode {
  ...
  toString(format = null) {
    return stringifySyntaxNodeOrTokenArray(this.children.map(c => c.toString(format)))
  }
   ...
   */
  toFullString(format = null) {
    return stringifySyntaxNodeOrTokenArray(this.children.map(c => c.toFullString(format)))
  }
}
