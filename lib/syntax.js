 ...
 * @typedef {TokenLike | string} TokenPattern
 */
...
class SyntaxNode {
  ...
  constructor(...nodesOrTokens) {
    ...
    this.tokens.forEach(t => console.assert(t && typeof t === 'object' && !(t instanceof SyntaxNode) && !(t instanceof TokenGroup)))
  }
  ...
  push(...nodesOrTokens) {
    for (const nodeOrToken of nodesOrTokens.flat()) {
      if (nodeOrToken instanceof SyntaxNode) {
        ...
        throw "no Array"
      } else if (nodeOrToken instanceof TokenIterator) {
        throw "no TokenIterator"
      } else if (nodeOrToken) {
