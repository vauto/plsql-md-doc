...
const { flattenTokens, isTokenLike, isToken, mustBeToken, Token, TokenIterator, TokenLike, TokenSyntaxError, stringifyTokenArray } = require("./token")
/**
...
class SyntaxNode {
  ...
  static from(value) {
    ...
    return new SyntaxNode(mustBeToken(value))
  }
...
class ContainerSyntaxNode extends SyntaxNode {
  ...
  push(...nodesOrTokens) {
    ...
    for (const nodeOrToken of nodesOrTokens.flat()) {
      ...
      console.warn(`${Object.values(nodeOrToken).at(0)?.start ?? this.start} named params`, this.constructor.name, Object.keys(nodeOrToken).sort())
