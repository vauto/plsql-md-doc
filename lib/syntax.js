...
class ExpressionSyntaxNode extends ContainerSyntaxNode {
  ...
  constructor(params = {}) {
    ...
    for (const [key, value] of Object.entries(params)) {
      try {
        const nodeOrNull = this[key] = SyntaxNode.asSyntaxNode(value)
        if (nodeOrNull) {
  ...
  // /**
  //  *
  //  * @param  {...SyntaxNodeOrToken|[key: string]: SyntaxNodeOrToken} nodesOrTokens
  //  */
  // push(...nodesOrTokens) {
...
const ebil = flattenTokens([[[[[{ 'type': 'foo' }, { 'type': 'bar' }]], { 'type': 'baz' }]]])
console.assert(ebil.length === 3 && ebil.every(isToken))
